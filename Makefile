...
test-e2e:  ## 运行端到端测试
	$(PYTEST) tests/acceptance/test_e2e_workflow.py tests/acceptance/test_gateway_startup.py tests/acceptance/test_mcp_e2e_full_workflow.py -v

test-mcp-e2e:  ## MCP 全功能端到端验收（仅通过 /mcp）
	$(PYTEST) tests/acceptance/test_mcp_e2e_full_workflow.py -v
...
check-local-ci-smoke:  ## 本地 CI wrapper 与 OpenMemory launcher 轻量回归（无需数据库）
	@echo "检查本地 CI smoke..."
	$(PYTEST) scripts/tests/test_ci_no_make.py tests/ci/test_start_openmemory_launcher.py -q
	@echo "本地 CI smoke 检查通过"
...
recover-openmemory-init-race:  ## 自愈 OpenMemory 启动期 pg_type 竞态（清空 openmemory schema 后让 migrator 重建）
	@bash scripts/ci/recover_openmemory_init_race.sh

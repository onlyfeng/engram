...
class TrimConfig:
    ...
    # diff/log 检测模式
    diff_pattern: re.Pattern = field(
        default_factory=lambda: re.compile(r"^[-+]{3}\s|^@@\s|^diff\s--git|^Index:\s", re.MULTILINE)
    )
    log_pattern: re.Pattern = field(
...
class MemoryCard:
    ...
    def _trim_text(self, text: str, max_length: int, field_name: str) -> TrimResult:
        """裁剪文本到指定长度"""
    ...
    def _trim_diff_or_log(
        self, text: str, content_type: str, uri: Optional[str] = None, sha256: Optional[str] = None
    ) -> str:
    ...
    def to_markdown(self) -> str:
        ...
        if len(markdown) > config.max_total_length:
            ...
            self._trim_logs.append(f"总长度截断: -> {config.max_total_length}")

...
class PolicyEngine:
    ...
    DEFAULT_POLICY = {
        ...
        "require_evidence": True,  # 是否强制证据链（存在性检查）
        "evidence_mode": "compat",  # 证据验证模式: compat / strict
    ...
    def _check_team_policy(
        ...
        if is_bulk:
            ...
            bulk_max_chars: int = self.policy.get("bulk_max_chars", 200)
            if bulk_mode == "very_short" and len(payload_md) > bulk_max_chars:
                return PolicyDecision(
                    action=PolicyAction.REDIRECT,
...
def create_engine_from_settings(settings: Dict[str, Any]) -> PolicyEngine:
    ...
    Args:
        settings: 从 LogbookDatabase.get_settings() 获取的设置

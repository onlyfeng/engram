...
class DegradationConfig:
    ...
    def from_config(cls, config: Optional[Any] = None) -> "DegradationConfig":
        ...
        return cls(
            ...
            min_forward_window_seconds=config.get(
                "scm.degradation.min_forward_window_seconds", 300
            ),
            max_forward_window_seconds=config.get(
                "scm.degradation.max_forward_window_seconds", 86400
            ),
            default_forward_window_seconds=config.get(
                "scm.degradation.default_forward_window_seconds", 3600
            ),
            ...
            content_too_large_threshold=config.get(
                "scm.degradation.content_too_large_threshold", 5
            ),
...
class DegradationController:
    ...
    def __init__(
        ...
        self._current_forward_window = (
            initial_forward_window_seconds or self._config.default_forward_window_seconds
        )
    ...
    def _classify_errors(self, unrecoverable_errors: List[Dict[str, Any]]) -> Dict[str, int]:
        ...
        for error in unrecoverable_errors:
            ...
            elif category in ("http_error", "server_error") or (
    ...
    def _calculate_sleep(
        self, error_counts: Dict[str, int], retry_after: Optional[float] = None
    ) -> float:
        """
        计算下一轮前应等待的秒数
    ...
    def _adjust_diff_mode(self, reasons: List[str]) -> str:
        ...
        if rate_limit_count >= self._config.rate_limit_threshold:
            if new_mode != "none":
                new_mode = "none"
                reasons.append(
                    f"rate_limit_count={rate_limit_count}>=threshold={self._config.rate_limit_threshold}"
                )
        ...
        if content_large_count >= self._config.content_too_large_threshold:
            if new_mode != "none":
                new_mode = "none"
                reasons.append(
                    f"content_too_large_count={content_large_count}>=threshold={self._config.content_too_large_threshold}"
                )
                logger.warning(
                    f"连续 content_too_large 达到阈值 ({content_large_count})，diff_mode 降级为 none"
                )

        # 检查是否可以恢复
        if (
            new_mode == "none"
            and self._consecutive_success_count >= self._config.recovery_success_count
        ):
            new_mode = "best_effort"
            reasons.append(
                f"consecutive_success={self._consecutive_success_count}>=recovery_threshold={self._config.recovery_success_count}"
            )
            logger.info(
                f"连续成功 {self._consecutive_success_count} 次，diff_mode 恢复为 best_effort"
            )
    ...
    def _adjust_batch_size(self, error_counts: Dict[str, int], reasons: List[str]) -> int:
        ...
        if rate_limit_count > 0 or timeout_count > 0:
            ...
            if new_batch_size != self._current_batch_size:
                reasons.append(
                    f"batch_size shrink: {self._current_batch_size}->{new_batch_size} (429={rate_limit_count}, timeout={timeout_count})"
                )
        ...
        # 连续成功，尝试恢复 batch_size
            ...
            if self._current_batch_size < self._config.default_batch_size:
                ...
                if new_batch_size != self._current_batch_size:
                    reasons.append(
                        f"batch_size grow: {self._current_batch_size}->{new_batch_size} (consecutive_success={self._consecutive_success_count})"
                    )
    ...
    def _adjust_forward_window(self, error_counts: Dict[str, int], reasons: List[str]) -> int:
        ...
        if rate_limit_count > 0:
            ...
            if new_window != self._current_forward_window:
                reasons.append(
                    f"forward_window shrink: {self._current_forward_window}->{new_window}"
                )
            ...
            if self._current_forward_window < self._config.default_forward_window_seconds:
                ...
                if new_window != self._current_forward_window:
                    reasons.append(
                        f"forward_window grow: {self._current_forward_window}->{new_window}"
                    )
    ...
    def _check_pause_condition(self, error_counts: Dict[str, int]) -> tuple:
        ...
        if timeout_count >= self._config.timeout_threshold:
            return (
                True,
                f"consecutive_timeout={timeout_count}>=threshold={self._config.timeout_threshold}",
            )

        if server_error_count >= self._config.timeout_threshold:
            return (
                True,
                f"consecutive_server_error={server_error_count}>=threshold={self._config.timeout_threshold}",
            )
    ...
    def update(
        ...
        if should_pause:
            # 设置暂停时间
            pause_duration = min(
                self._config.base_sleep_seconds
                * (2 ** self._consecutive_errors.get(ErrorType.TIMEOUT.value, 1)),
...
class SchedulerConfig:
    ...
    job_type_priority: Dict[str, int] = field(
    ...
    def from_config(cls, config: Optional[Any] = None) -> "SchedulerConfig":
        ...
        max_running = _get_env_or_config(
            "SCM_SCHEDULER_MAX_RUNNING", "scm.scheduler.max_running", 5
        )

        # global_concurrency 环境变量覆盖 max_queue_depth
        max_queue_depth = _get_env_or_config(
            "SCM_SCHEDULER_GLOBAL_CONCURRENCY", "scm.scheduler.max_queue_depth", 10
        )
        if config is not None:
        ...
        return cls(
            ...
            error_budget_window_size=_get_env_or_config(
                ...
                "scm.scheduler.error_budget_window_size",
                10,
            ),
            rate_limit_hit_threshold=_get_env_or_config(
                "SCM_SCHEDULER_RATE_LIMIT_HIT_THRESHOLD",
                "scm.scheduler.rate_limit_hit_threshold",
                0.1,
                value_type=float,
            ),
            pause_duration_seconds=_get_env_or_config(
                "SCM_SCHEDULER_PAUSE_DURATION_SECONDS", "scm.scheduler.pause_duration_seconds", 300
            ),
            scan_interval_seconds=_get_env_or_config(
                "SCM_SCHEDULER_SCAN_INTERVAL_SECONDS", "scm.scheduler.scan_interval_seconds", 60
            ),
            max_enqueue_per_scan=_get_env_or_config(
                "SCM_SCHEDULER_MAX_ENQUEUE_PER_SCAN", "scm.scheduler.max_enqueue_per_scan", 100
            ),
            # Tenant 公平调度配置
            ...
            mvp_mode_enabled=_get_env_or_config(
                ...
                "scm.scheduler.mvp_mode_enabled",
                False,
                value_type=bool,
            ),
...
class PauseSnapshot:
    ...
    记录调度器扫描时刻从 logbook.kv (namespace='scm.sync_pauses')
    读取的暂停状态。

    设计说明:
    - paused_pairs 仅表示 kv 中的暂停记录，与 queued_pairs（DB 中 active jobs）分离
    - 调度器在 select_jobs_to_enqueue 之前构造此快照
    - 策略层内部将 paused_pairs 和 queued_pairs 合并进行过滤

    Attributes:
        paused_pairs: 当前暂停的 (repo_id, job_type) 集合
        pause_count: 暂停记录总数
        by_reason_code: 按 reason_code 分组的暂停计数
        snapshot_at: 快照时间戳（Unix timestamp）
    """

    paused_pairs: set = field(default_factory=set)
    pause_count: int = 0
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            "paused_pairs": list(self.paused_pairs),
    ...
    @classmethod
    def empty(cls) -> "PauseSnapshot":
        """创建一个空的暂停快照"""
        return cls()

    @classmethod
    def from_pairs(cls, pairs: set, *, snapshot_at: Optional[float] = None) -> "PauseSnapshot":
        """从 paused_pairs 集合创建快照（简化构造）"""
        return cls(
            paused_pairs=set(pairs),
            pause_count=len(pairs),
            snapshot_at=snapshot_at or time.time(),
        )
...
def select_jobs_to_enqueue(
    ...
    paused_pairs: Optional[set] = None,
    ...
    3. 按 (repo_id, job_type) 组合判断是否已在队列或被暂停，避免重复入队
    ...
    队列与暂停的分离设计:
    - queued_pairs: 仅表示 DB 中 active jobs (status IN ('pending', 'running'))
    - paused_pairs / pause_snapshot: 仅表示 kv 中的暂停记录 (namespace='scm.sync_pauses')
    - 函数内部将两者合并进行过滤，调用方无需手动合并
    ...
    Args:
        ...
        skip_on_bucket_pause: 如果为 True，bucket 暂停时跳过任务；
                             如果为 False（默认），仅降权不跳过
        paused_pairs: 当前暂停的 (repo_id, job_type) 集合（来自 kv 暂停记录）
                     若为 None 则不过滤暂停任务
        pause_snapshot: 暂停状态快照，包含 paused_pairs 和统计信息
                       若提供，则使用 pause_snapshot.paused_pairs 覆盖 paused_pairs
    ...
    # 处理 paused_pairs：优先使用 pause_snapshot.paused_pairs
    effective_paused_pairs: set = set()
    if pause_snapshot is not None:
        effective_paused_pairs = pause_snapshot.paused_pairs
    elif paused_pairs is not None:
        effective_paused_pairs = paused_pairs

    # 合并 queued_pairs 和 paused_pairs 形成跳过集合
    skip_pairs = queued_pairs | effective_paused_pairs
            ...
            if config.mvp_mode_enabled:
                if job_type not in config.mvp_job_type_allowlist:
                    logger.debug(
                        ...
                        state.repo_id,
                        job_type,
                        config.mvp_job_type_allowlist,
                    )
                    continue

            # 检查此 (repo_id, job_type) 是否已在队列中或被暂停
            if (state.repo_id, job_type) in skip_pairs:
                continue  # 跳过已入队或被暂停的任务
    ...
    def from_config(cls, config: Optional[Any] = None) -> "CircuitBreakerConfig":
        ...
        return cls(
            failure_rate_threshold=_get_env_or_config(
                "SCM_CB_FAILURE_RATE_THRESHOLD",
                "scm.circuit_breaker.failure_rate_threshold",
                0.3,
                value_type=float,
            ),
            rate_limit_threshold=_get_env_or_config(
                "SCM_CB_RATE_LIMIT_THRESHOLD",
                "scm.circuit_breaker.rate_limit_threshold",
                0.2,
                value_type=float,
            ),
            timeout_rate_threshold=_get_env_or_config(
                "SCM_CB_TIMEOUT_RATE_THRESHOLD",
                "scm.circuit_breaker.timeout_rate_threshold",
                0.2,
                value_type=float,
            ),
            # 小样本保护参数
            min_samples=_get_env_or_config(
                "SCM_CB_MIN_SAMPLES", "scm.circuit_breaker.min_samples", 5
            ),
            # 平滑策略参数
            enable_smoothing=_get_env_or_config(
                "SCM_CB_ENABLE_SMOOTHING",
                "scm.circuit_breaker.enable_smoothing",
                True,
                value_type=bool,
            ),
            smoothing_alpha=_get_env_or_config(
                "SCM_CB_SMOOTHING_ALPHA",
                "scm.circuit_breaker.smoothing_alpha",
                0.5,
                value_type=float,
            ),
            window_count=_get_env_or_config(
                "SCM_CB_WINDOW_COUNT", "scm.circuit_breaker.window_count", 20
            ),
            window_minutes=_get_env_or_config(
                "SCM_CB_WINDOW_MINUTES", "scm.circuit_breaker.window_minutes", 30
            ),
            open_duration_seconds=_get_env_or_config(
                "SCM_CB_OPEN_DURATION_SECONDS", "scm.circuit_breaker.open_duration_seconds", 300
            ),
            half_open_max_requests=_get_env_or_config(
                "SCM_CB_HALF_OPEN_MAX_REQUESTS", "scm.circuit_breaker.half_open_max_requests", 3
            ),
            recovery_success_count=_get_env_or_config(
                "SCM_CB_RECOVERY_SUCCESS_COUNT", "scm.circuit_breaker.recovery_success_count", 2
            ),
            degraded_batch_size=_get_env_or_config(
                "SCM_CB_DEGRADED_BATCH_SIZE", "scm.circuit_breaker.degraded_batch_size", 10
            ),
            degraded_forward_window_seconds=_get_env_or_config(
                "SCM_CB_DEGRADED_FORWARD_WINDOW_SECONDS",
                "scm.circuit_breaker.degraded_forward_window_seconds",
                300,
            ),
            backfill_only_mode=_get_env_or_config(
                "SCM_CB_BACKFILL_ONLY_MODE",
                "scm.circuit_breaker.backfill_only_mode",
                True,
                value_type=bool,
            ),
            backfill_interval_seconds=_get_env_or_config(
                "SCM_CB_BACKFILL_INTERVAL_SECONDS",
                "scm.circuit_breaker.backfill_interval_seconds",
                600,
            ),
            probe_budget_per_interval=_get_env_or_config(
                "SCM_CB_PROBE_BUDGET_PER_INTERVAL",
                "scm.circuit_breaker.probe_budget_per_interval",
                2,
            ),
...
class CircuitBreakerController:
    ...
    def _should_trip(self, health_stats: Dict[str, Any]) -> tuple:
        ...
        if total_runs < min_samples:
            logger.debug(
                "熔断检查跳过（样本不足）: total_runs=%d < min_samples=%d", total_runs, min_samples
            )
            return (False, None)
        ...
        smoothed_rate_limit_rate = self._apply_smoothing(
            raw_rate_limit_rate, self._smoothed_rate_limit_rate
        )
        ...
        if rate_limit_rate >= self._config.rate_limit_threshold:
            ...
            if self._config.enable_smoothing:
                reason += (
                    f" (raw={raw_rate_limit_rate:.2%}, smoothed={smoothed_rate_limit_rate:.2%})"
                )
            return (True, reason)

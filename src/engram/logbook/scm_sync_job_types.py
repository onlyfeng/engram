"""
engram_logbook.scm_sync_job_types - SCM 同步 job_type 归一化模块
...
- reviews: Merge Request Review 事件同步（仅 Git）
...
class LogicalJobType(str, Enum):
    ...
    REVIEWS = "reviews"  # MR Reviews（仅 Git）
...
class PhysicalJobType(str, Enum):
    ...
    GITLAB_REVIEWS = "gitlab_reviews"  # GitLab MR Reviews
    SVN = "svn"  # SVN 提交记录
...
def physical_to_logical(physical_job_type: str) -> str:
    ...
    raise ValueError(
        f"无效的 physical_job_type='{physical_job_type}'，有效值: {list(mapping.keys())}"
    )
...
def normalize_job_type(
    ...
    if normalized in ALL_LOGICAL_JOB_TYPES:
        if repo_type is None:
            raise ValueError(f"job_type='{job_type}' 是 logical 类型，需要提供 repo_type 进行转换")

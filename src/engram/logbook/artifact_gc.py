#!/usr/bin/env python3
"""
engram.logbook.artifact_gc - 制品垃圾回收核心实现
...
使用方式:
    from engram.logbook.artifact_gc import run_gc, run_tmp_gc, GCResult

    result = run_gc(prefix="artifacts/", dry_run=True)
...
def run_gc(
    *,
    ...
    Args:
        ...
        delete: 是否执行删除
        dsn: 数据库连接字符串
        backend: 存储后端类型 (local/file/object)
        artifacts_root: 制品存储根目录
        allowed_prefixes: 允许的前缀列表
        older_than_days: 最小文件年龄（天）
        trash_prefix: 垃圾箱前缀（移动而非删除）
        limit: 最大扫描文件数
        require_ops: 是否需要 ops 凭证
        require_trash: 是否必须使用垃圾箱
        audit: 是否记录审计日志
        verbose: 是否输出详细信息
    ...
    try:
        referenced = (
            get_referenced_uris(dsn=dsn, prefix=prefix) if dsn else ReferencedUris(set(), [])
        )

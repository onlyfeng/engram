...
def redact_dict(
    ...
    for key, value in data.items():
        ...
        if key_lower in all_sensitive_keys:
            ...
            result[key] = [
                redact_dict(item, sensitive_keys, deep)
...
class StaticTokenProvider(TokenProvider):
    ...
    def invalidate(self) -> None:
        # 静态 token 无法刷新，仅记录警告
        logger.warning("StaticTokenProvider: invalidate() 被调用，但静态 token 无法刷新")
...
class EnvTokenProvider(TokenProvider):
    ...
    def get_token(self) -> str:
        ...
        logger.debug(f"EnvTokenProvider: 从 ${self._env_var} 读取 token: {mask_token(token)}")

        return token

    def invalidate(self) -> None:
        logger.info(f"EnvTokenProvider: token 失效标记，下次获取将重新读取 ${self._env_var}")
...
class FileTokenProvider(TokenProvider):
    ...
    def get_token(self) -> str:
        ...
        self._cache = CachedToken(value=token, fetched_at=now)
        logger.debug(f"FileTokenProvider: 从 {self._file_path} 读取 token: {mask_token(token)}")

        return token

    def invalidate(self) -> None:
        logger.info(f"FileTokenProvider: token 失效标记，下次获取将重新读取 {self._file_path}")
        self._invalidated = True
...
class ExecTokenProvider(TokenProvider):
    ...
    def get_token(self) -> str:
        ...
        try:
            ...
            self._cache = CachedToken(value=token, fetched_at=now)
            logger.debug(f"ExecTokenProvider: 从命令获取 token: {mask_token(token)}")

            return token

        except subprocess.TimeoutExpired:
            raise TokenValidationError(f"Token 命令超时 ({self._timeout}s): {self._command}")

    def invalidate(self) -> None:
        logger.info("ExecTokenProvider: token 失效标记，下次获取将重新执行命令")
        self._invalidated = True
    ...
    if config:
        ...
        if token_env:
            min_interval = config.get(
                "scm.gitlab.token_refresh_interval", DEFAULT_MIN_REFRESH_INTERVAL
            )
...
def _normalize_instance_key_for_config(instance_key: str) -> str:
    ...
    Note:
        此函数是内部使用的配置键规范化函数。
        对于外部使用，请使用 engram_logbook.scm_sync_keys.normalize_instance_key
    """
    if not instance_key:
        return ""
    # 替换常见分隔符为下划线
        ...
        while True:
            ...
            if response.status_code in (401, 403):
                if retries < self._max_auth_retries:
                    logger.warning(f"收到 {response.status_code} 响应，触发 token 刷新并重试")

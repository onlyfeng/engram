...
JSON 边界约定:
- 所有 *_json 参数接受结构化 Python 对象（dict/list/str/int/None）
- DB 层负责在入库时调用 json.dumps()
- 上层代码不应传入已序列化的 JSON 字符串
"""
...
class ItemWithLatestEventRow(TypedDict):
    """带最近事件的 item 返回行类型定义"""
...
class KnowledgeCandidateRow(TypedDict):
    """knowledge_candidates 表返回行的类型定义"""
    ...
    evidence_refs_json: list[Any] | None
    promote_suggested: bool | None
    created_at: datetime


# JSON 字段类型别名：限制 JSON 可序列化类型
...
class Database:
    ...
    def __init__(self, dsn: str | None = None, config: Config | None = None):
        """
        初始化数据库连接管理器
    ...
    def get_kv(self, key: str, namespace: str | None = None) -> JsonValue:
        """获取 KV"""
        ns = namespace or self.DEFAULT_KV_NAMESPACE
        return get_kv(
            namespace=ns,
...
def get_database(config: Config | None = None, reload: bool = False) -> Database:
    """
    获取全局数据库实例
    ...
    if _global_database is None or reload:
        _global_database = Database(dsn=None, config=config)
...
def get_dsn(config: Config | None = None) -> str:
    """
    从配置中获取数据库 DSN
    ...
    # 优先级 1: 显式配置
    dsn_config = config.get("postgres.dsn")
    if dsn_config:
        return str(dsn_config)
        ...
        if search_path_cfg:
            ...
            try:
                ...
                if global_ctx.schema_prefix is not None:
                    schemas = list(global_ctx.search_path)
            except Exception:
                pass
...
def execute_sql_file(
    ...
    Args:
        ...
        placeholders: 占位符替换字典，如 {"__OPENMEMORY_SCHEMA__": "myproj_openmemory"}
                     key 和 value 必须都是字符串，占位符将在执行前被替换为对应的值
...
def create_item(
    item_type: str,
    title: str,
    scope_json: JsonObject | None = None,
    status: str = "open",
...
def add_event(
    ...
    Args:
        item_id: 条目 ID
        event_type: 事件类型
        payload_json: 事件负载（结构化对象，DB 层负责序列化）
    ...
    if payload_json is None and payload is not None:
        payload_json = payload
    payload_data: JsonObject = payload_json or {}
...
def attach(
    ...
    meta_json: JsonObject | None = None,
    config: Config | None = None,
    dsn: str | None = None,
) -> int:
    """
    ...
    try:
        with conn.cursor() as cur:
            ...
            if result is None:
                raise DatabaseError(
                    "添加附件失败: 未返回 attachment_id",
                    {"item_id": item_id, "kind": kind, "uri": uri},
                )
            return int(result[0])
    except psycopg.Error as e:
        conn.rollback()
...
def get_items_with_latest_event(
    ...
    try:
        with conn.cursor() as cur:
            ...
            results: list[ItemWithLatestEventRow] = []
            for row in rows:
                results.append(
                    ItemWithLatestEventRow(
                        item_id=row[0],
                        item_type=row[1],
                        title=row[2],
                        scope_json=row[3],
...
def query_knowledge_candidates(
    ...
    try:
        with conn.cursor() as cur:
            ...
            results: list[KnowledgeCandidateRow] = []
            for row in rows:
                results.append(

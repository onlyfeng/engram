"""
engram_logbook.config - 配置管理模块

支持:
- CLI --config 参数覆盖
- 环境变量 ENGRAM_LOGBOOK_CONFIG 指定配置文件路径
- TOML 格式配置文件

优先级: --config > ENGRAM_LOGBOOK_CONFIG > ./.agentx/config.toml > ~/.agentx/config.toml
"""
            ...
            if not path.exists():
                raise ConfigNotFoundError(
                    f"指定的配置文件不存在: {explicit_path}",
                    {
                        ...
                        "hint": (
                            "请检查路径是否正确，或使用以下方式之一配置:\n"
                            f"  1. 创建配置文件: mkdir -p {path.parent} && cp templates/config.example.toml {path}\n"
                            f"  2. 使用环境变量: export {ENV_CONFIG_PATH}=/path/to/config.toml\n"
                            "  3. 使用默认路径: ./.agentx/config.toml 或 ~/.agentx/config.toml"
                        ),
                    },
                )
            ...
            if not path.exists():
                raise ConfigNotFoundError(
                    f"环境变量 {ENV_CONFIG_PATH} 指定的配置文件不存在: {env_path}",
                    {
                        ...
                        "hint": (
                            f"环境变量 {ENV_CONFIG_PATH} 设置为 '{env_path}'，但该文件不存在。\n"
                            "请选择以下方式之一修复:\n"
                            f"  1. 创建配置文件: mkdir -p {path.parent} && cp templates/config.example.toml {path}\n"
                            f"  2. 修改环境变量: export {ENV_CONFIG_PATH}=/path/to/existing/config.toml\n"
                            f"  3. 取消环境变量: unset {ENV_CONFIG_PATH}"
                        ),
                    },
                )
            self._config_path = path
...
# SVN 每个 revision 估算的秒数（用于窗口限制校验）
# 默认假设每个 revision 平均间隔 1 小时
DEFAULT_SVN_SECONDS_PER_REV = 3600


def estimate_svn_window_seconds(
    revision_count: int,
    seconds_per_rev: int = DEFAULT_SVN_SECONDS_PER_REV,
) -> int:
    """
    估算 SVN 回填窗口秒数
    ...
    if revision_count <= 0:
        return 0
    return revision_count * seconds_per_rev
...
# SCM Sync 开关：默认关闭，不影响 Logbook 最小闭环 (T01)
# 设为 True 后，SCM 同步功能（scheduler/worker/reaper）才会启用
ENV_SCM_SYNC_ENABLED = "ENGRAM_SCM_SYNC_ENABLED"
DEFAULT_SCM_SYNC_ENABLED = False
...
def is_scm_sync_enabled(config: Optional["Config"] = None) -> bool:
    ...
    """
    # 1. 优先环境变量
    env_val = os.environ.get(ENV_SCM_SYNC_ENABLED, "").strip().lower()
...
def require_scm_sync_enabled(component_name: str = "SCM Sync") -> None:
    ...
    """
    if not is_scm_sync_enabled():
        raise ConfigError(
            f"{component_name} 功能未启用",
            {
                "component": component_name,
                "hint": (
                    f"{component_name} 是可选功能，默认关闭以不影响 Logbook 最小闭环 (T01)。\n"
                    "如需启用，请选择以下方式之一:\n\n"
                    "1. 环境变量:\n"
                    f"   export {ENV_SCM_SYNC_ENABLED}=true\n\n"
                    "2. 配置文件:\n"
                    "   [scm.sync]\n"
                    "   enabled = true\n"
                ),
...
# GC 治理开关环境变量
ENV_GC_REQUIRE_TRASH_DEFAULT = "ENGRAM_GC_REQUIRE_TRASH_DEFAULT"
ENV_GC_REQUIRE_OPS_DEFAULT = "ENGRAM_GC_REQUIRE_OPS_DEFAULT"

# GC 治理开关默认值
DEFAULT_GC_REQUIRE_TRASH = False
DEFAULT_GC_REQUIRE_OPS = False
...
def get_gc_governance_config(config: Optional["Config"] = None) -> dict:
    ...
    Returns:
        ...
        - require_ops_default: bool, 是否默认要求 ops 凭证（仅 object 后端）
    """
    if config is None:
    ...
    if env_require_trash is not None:
        ...
        if cfg_value is not None:
            if isinstance(cfg_value, bool):
                require_trash_default = cfg_value
            elif isinstance(cfg_value, str):
                require_trash_default = cfg_value.lower() in ("true", "1", "yes", "on")
            else:
    ...
    if env_require_ops is not None:
        ...
        cfg_value = config.get("gc.require_ops_default")
        if cfg_value is not None:
            if isinstance(cfg_value, bool):
                require_ops_default = cfg_value
            elif isinstance(cfg_value, str):
                require_ops_default = cfg_value.lower() in ("true", "1", "yes", "on")
            else:

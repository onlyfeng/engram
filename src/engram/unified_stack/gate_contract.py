# -*- coding: utf-8 -*-
"""
gate_contract.py - Unified Stack 门禁配置/能力模型
...
使用方法:
  # Python 侧
  from engram.unified_stack import (
      ProfileType,
      detect_capabilities,
      validate_profile,
  )
...
def detect_capabilities() -> CapabilityReport:
    ...
    if endpoint_present:
        endpoint_message = f"OpenMemory endpoint available via {endpoint_var_name}={endpoint_value}"
    else:
        endpoint_message = (
    ...
    capabilities["openmemory_endpoint_present"] = CapabilityStatus(
        ...
        if endpoint_present
        else ReasonCode.CAP_OPENMEMORY_ENDPOINT_MISSING,

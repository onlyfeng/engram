...
import { validateDbName, quoteIdentifier } from "./utils/db_security";

// 重新导出供外部使用
export { validateDbName, quoteIdentifier };
...
if (is_pg) {
    ...
    // 示例: OM_PG_SET_ROLE=openmemory_app 或留空不切换角色
        ...
        if (setRole) {
            p.on('connect', async (client) => {
                try {
                    ...
                    console.error(`[DB] Failed to SET ROLE "${setRole}": ${e.message}`);
                }
            });
        }
        return p;
    ...
    let pg = pool(db_name);
    if (setRole) {
        console.error(`[DB] OM_PG_SET_ROLE configured: ${setRole}`);
    }
    ...
    // OM_PG_AUTO_DDL 控制是否在运行时自动建表/建索引
    // 默认为 false，生产环境建议使用 npm run migrate 预先初始化 schema
    // 设置为 true 可在开发环境自动建表（需要 CREATE 权限）
    const auto_ddl = process.env.OM_PG_AUTO_DDL === "true";
    
    // OM_PG_AUTO_CREATE_DB 控制是否自动创建数据库（默认 false）
    // 生产环境应使用 npm run migrate 或手动创建数据库
    // 仅在开发环境明确设置为 true 时启用
    const auto_create_db = process.env.OM_PG_AUTO_CREATE_DB === "true";
    
    const init = async () => {
        // 预先校验数据库名称
        const [valid, error] = validateDbName(db_name);
        if (!valid) {
            console.error(`[DB] Invalid database name: ${error}`);
            throw new Error(`Invalid database name: ${error}`);
        }
        ...
        try {
            ...
            if (err.code === "3D000") {
                // 数据库不存在
                if (!auto_create_db) {
                    console.error(`[DB] Database "${db_name}" does not exist.`);
                    console.error(`[DB] Please run 'npm run migrate' first or set OM_PG_AUTO_CREATE_DB=true`);
                    throw err;
                }
                
                console.error(`[DB] OM_PG_AUTO_CREATE_DB=true, attempting to create database...`);
                ...
                try {
                    ...
                    console.error(`[DB] Created database: ${db_name}`);
                } catch (e: any) {
                    // 42P04 = database already exists（并发创建场景）
                    if (e.code !== "42P04") throw e;
                    console.error(`[DB] Database ${db_name} already exists (concurrent creation)`);
                } finally {
                    await admin.end();
                }
                pg = pool(db_name);
        ...
        // 仅当 OM_PG_AUTO_DDL=true 时执行建表/建索引
        // 生产环境应使用 npm run migrate 预先初始化
        if (auto_ddl) {
            console.error(`[DB] OM_PG_AUTO_DDL=true, auto-creating tables/indexes...`);
            await pg.query(
            ...
            console.error(`[DB] OM_PG_AUTO_DDL=false (default), skipping auto DDL`);
            console.error(`[DB] Tables should be created via 'npm run migrate'`);
        }

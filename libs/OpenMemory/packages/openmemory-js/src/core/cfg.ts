...
/**
 * 解析 API keys，支持以下配置方式：
 * 1. OM_API_KEYS: 逗号分隔的多个 key
 * 2. OM_API_KEY + OM_API_KEY_NEXT: 两个独立 key（用于轮换）
 * 3. OM_API_KEY: 单个 key（向后兼容）
 */
const parse_api_keys = (): string[] => {
    const keys: string[] = [];
    // 优先使用 OM_API_KEYS（逗号分隔）
    if (process.env.OM_API_KEYS) {
        keys.push(
            ...process.env.OM_API_KEYS.split(",")
                .map((k) => k.trim())
                .filter(Boolean),
        );
    }
    // 回退到 OM_API_KEY + OM_API_KEY_NEXT
    if (keys.length === 0) {
        if (process.env.OM_API_KEY) keys.push(process.env.OM_API_KEY);
        if (process.env.OM_API_KEY_NEXT) keys.push(process.env.OM_API_KEY_NEXT);
    }
    return keys;
};
...
export const env = {
    ...
    api_key: process.env.OM_API_KEY, // 向后兼容
    api_keys: parse_api_keys(), // 多 key 支持

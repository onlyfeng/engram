...
Logbook 用 Postgres 把"事实层"做成团队基础设施：
...
## Logbook 与 Memory Gateway / SeekDB 的边界

> **术语说明**：Memory Gateway 简称 Gateway，SeekDB 简称 Seek。详见 [命名规范](../architecture/naming.md)。

- Logbook：真相源（SoT），可审计、可回放、可校验
- Memory Gateway（OpenMemory MCP 网关）：经验/记忆层（可演化，可强化/衰减），默认团队可读；团队可写受开关与策略治理
- SeekDB（证据索引/RAG 索引）：证据检索加速层（对大文本/代码/报告分块索引），与 Gateway 组合实现"先策略后证据"

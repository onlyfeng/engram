# 验收证据片段模板

> **使用说明**：本模板展示 `## 验收证据` 段落的推荐结构。
> 实际内容应由 `render_iteration_evidence_snippet.py` 脚本读取 evidence JSON 自动渲染生成。
>
> **重要**：回归文档应放置于 `docs/acceptance/` 目录下，证据文件放置于 `docs/acceptance/evidence/` 目录下。受控区块（`AUTO-GENERATED ...` markers）内容与 marker 禁止手动编辑，evidence JSON 必须由脚本生成，禁止手工创建或修改。

---

## 验收证据

<!-- AUTO-GENERATED EVIDENCE BLOCK START -->
<!-- AUTO-GENERATED BY render_iteration_evidence_snippet.py -->
<!-- 受控区块：禁止手动修改内容或 marker -->

| 项目 | 值 |
|------|-----|
| **证据文件** | [`iteration_13_evidence.json`](evidence/iteration_13_evidence.json) |
| **Schema 版本** | `iteration_evidence_v2.schema.json` |
| **记录时间** | 2026-02-02T14:30:22Z |
| **Commit SHA** | `abc1234` |

### 门禁命令执行摘要

> 以下表格由脚本从 `evidence/*.json` 的 `commands` 数组自动渲染。

| 命令 | 结果 | 耗时 | 摘要 |
|------|------|------|------|
| `make ci` | PASS | 45s | All checks passed |

### 整体验收结果

- **结果**: PASS
- **说明**: 所有门禁通过

<!-- AUTO-GENERATED EVIDENCE BLOCK END -->

---

### 生成命令

使用以下脚本自动渲染验收证据段落（推荐默认流程）：

```bash
# 生成证据文件
python scripts/iteration/record_iteration_evidence.py <N> \
  --ci-run-url https://github.com/<org>/<repo>/actions/runs/<run_id>

# 同步回归文档中的受控区块（min_gate_block + evidence_snippet）
python scripts/iteration/sync_iteration_regression.py <N> --write

# 仅预览/渲染 evidence 片段（可选）
python scripts/iteration/render_iteration_evidence_snippet.py <N>

# 仅渲染最小门禁块（可选）
python scripts/iteration/render_min_gate_block.py <N>
```

### 校验命令

```bash
python -m jsonschema -i docs/acceptance/evidence/iteration_<N>_evidence.json \
  schemas/iteration_evidence_v2.schema.json
```

---

_模板版本：v1.3 | 更新日期：2026-02-02（添加 render_iteration_evidence_snippet.py 脚本）_

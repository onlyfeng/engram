...
> 版本: v2.0  
> 生效日期: 2026-02-03  
> 关联契约: [`mcp_jsonrpc_error_v2.md`](./mcp_jsonrpc_error_v2.md)
...
| `JsonRpcErrorCode` | — | `mcp_rpc.py`（别名） | `= McpErrorCode`（保留定义） |
...
**现状**：
- `docs/contracts/mcp_jsonrpc_error_v2.md` §13.3.1 声明：代码（`mcp_rpc.py:ErrorReason`）为 reason 常量权威来源
- `docs/contracts/mcp_jsonrpc_error_v2.md` §13.3.2 声明：Schema 为结构权威
...
**需要改动的文件**：
1. `docs/contracts/mcp_jsonrpc_error_v2.md` - 更新 §13.3.1 表述
...
**需要改动的文件**：
1. `docs/contracts/mcp_jsonrpc_error_v2.md` - 新增豁免策略章节
...
**现状**：
- 文档 §13.5 声明已废弃
- `McpErrorCode.TOOL_EXECUTION_ERROR` 仍保留在代码中（按既有策略保留）
...
- [ ] `schemas/mcp_jsonrpc_error_v2.schema.json` - `definitions.error_reason.enum` 新增值
- [ ] `src/engram/gateway/error_codes.py` - `McpErrorReason` 新增常量
- [ ] `src/engram/gateway/error_codes.py` - `PUBLIC_MCP_ERROR_REASONS` 新增值
- [ ] `docs/contracts/mcp_jsonrpc_error_v2.md` - §4 对应分类表格新增行
...
- [ ] `docs/contracts/mcp_jsonrpc_error_v2.md` - §4.6 表格新增行
...
| 版本 | 日期 | 变更 |
|------|------|------|
| v2.0 | 2026-02-03 | 命名升级为 v2，更新 SSOT 引用 |

...
**行为说明**:
- `true`: 写入请求必须包含非空的 `evidence`（兼容 legacy `evidence_refs`）
- `false`: 允许无证据链的写入
- 证据缺失会触发 `missing_evidence` 原因的重定向

**Evidence 支持**（自 2024-01 起）:

策略引擎的 `decide()` 方法支持 `evidence_present` 参数，用于基于**规范化后的 evidence** 判断证据存在性：

| 场景 | evidence_refs (legacy) | evidence | evidence_present | 结果 |
|------|------------------------|----------|------------------|------|
| legacy 仅有 | `["uri"]` | `null` | `true`（自动计算） | 通过 |
| evidence 仅有 | `null` | `[{...}]` | `true`（自动计算） | 通过 |
| 两者皆有 | `["uri"]` | `[{...}]` | `true`（evidence 优先） | 通过 |
| 两者皆无 | `null` | `null` | `false` | 触发 missing_evidence |

**实现细节**:
- `memory_store_impl` 会先调用 `normalize_evidence()` 将 legacy/当前格式统一
- 然后计算 `evidence_present = len(normalized_evidence) > 0`
- 将 `evidence_present` 传递给 `PolicyEngine.decide()` 进行判断
- 这确保了 evidence + evidence_refs=None 时**不会**误触发 `missing_evidence`
...
**compat 模式行为**:
- 接受 legacy 格式 (`evidence_refs: List[str]`) 和当前格式 (`evidence: List[object]`)
- legacy 格式自动映射为 current external 格式（sha256 为空）
- 不校验 sha256 是否存在
...
| 字段 | 引入版本 | 向后兼容 | 迁移方式 |
|------|----------|----------|----------|
| `allowlist_users` | v2 | - | 原始字段 |
| `allowed_kinds` | v2 | - | 原始字段 |
| `require_evidence` | v2 | - | 原始字段 |
| `max_chars` | v2 | - | 原始字段 |
| `bulk_mode` | v2 | - | 原始字段 |
| `evidence_mode` | v2 | 是 | 默认 `compat`，无需迁移 |
| `bulk_max_chars` | v2 | 是 | 默认 `200`，无需迁移 |

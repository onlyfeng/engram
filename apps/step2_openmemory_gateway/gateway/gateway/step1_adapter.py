"""
step1_adapter - Step1 engram_step1 包适配器模块

提供对 engram_step1 包的封装，将 main/worker 需要的接口转发到
engram_step1.governance 与 engram_step1.outbox 模块。

此模块是 Step2 与 Step1 包之间的桥梁，统一接口调用方式。
"""
...
# 从 engram_step1 导入核心模块
from engram_step1 import governance, outbox
from engram_step1.config import Config, get_config
from engram_step1.db import get_connection, query_knowledge_candidates as _query_knowledge_candidates
from engram_step1.errors import DatabaseError
...
# 从 engram_step1.migrate 导入数据库检查和迁移函数
_DB_MIGRATE_AVAILABLE = False
try:
    from engram_step1.migrate import run_all_checks, run_migrate
...
class Step1DBCheckError(Exception):
    """Step1 DB 检查失败异常"""
    
    def __init__(self, message: str, missing_items: Dict[str, Any] = None):
        super().__init__(message)
        self.message = message
        self.missing_items = missing_items or {}


class Step1DBCheckResult:
    """Step1 DB 检查结果"""
...
class Step1Adapter:
    ...
    封装 engram_step1 包的功能，提供统一的接口给 Step2 的 main 和 worker 使用。
    """
    ...
    def __init__(self, dsn: Optional[str] = None, config: Optional[Config] = None):
        ...
        - 确保 engram_step1 的所有子模块都使用该 DSN
        
        Args:
            dsn: PostgreSQL 连接字符串，为 None 时从 POSTGRES_DSN 或 TEST_PG_DSN 环境变量读取
            config: engram_step1 Config 实例
        """
        self._config = config
        self._dsn = dsn
        
        # 如果显式提供了 dsn，使用强覆盖策略设置到环境变量
        # 确保 engram_step1 的 get_connection 等函数使用该 DSN
        if dsn:
    ...
    def get_or_create_settings(self, project_key: str) -> Dict[str, Any]:
        ...
        使用 engram_step1.governance.get_or_create_settings 实现，
    ...
    def check_db_schema(self) -> Step1DBCheckResult:
        """
        检查 Step1 数据库的 schema/表/索引/物化视图是否存在
        
        使用 db_migrate.run_all_checks 进行检查。
        
        Returns:
            Step1DBCheckResult 检查结果
        """
        if not _DB_MIGRATE_AVAILABLE:
            return Step1DBCheckResult(
                ok=False,
    ...
    def ensure_db_ready(
        ...
        """
        确保 Step1 DB 已就绪（schema/表/索引/物化视图存在）
        ...
        Raises:
            Step1DBCheckError: 如果 DB 结构缺失且无法自动修复
        """
        # 1. 执行检查
        ...
        if auto_migrate:
            ...
            raise Step1DBCheckError(
                message=f"Step1 DB 结构不完整: {check_result.get_missing_summary()}\n\n{repair_hint}",
                missing_items=check_result.checks,
            )


# 模块级别便捷函数
_adapter_instance: Optional[Step1Adapter] = None


def get_adapter(dsn: Optional[str] = None) -> Step1Adapter:
    """获取全局适配器实例"""
    global _adapter_instance
    if _adapter_instance is None:
        _adapter_instance = Step1Adapter(dsn)
...
def check_db_schema(dsn: Optional[str] = None) -> Step1DBCheckResult:
    """
    检查 Step1 数据库的 schema/表/索引/物化视图是否存在

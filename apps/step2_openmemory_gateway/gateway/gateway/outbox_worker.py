...
try:
    ...
    print(
        ...
        "=" * 60 + "\n"
        "\n"
        "Outbox Worker 依赖 engram_step1 模块，请先安装：\n"
        "\n"
...
class WorkerConfig:
    ...
    openmemory_max_client_retries: int = 0      # OpenMemory 客户端内部最大重试次数（0=不重试，由 Worker 层控制重试）
...
class ProcessResult:
    ...
    reason: str              # outbox_flush_success / outbox_flush_retry / outbox_flush_dead / outbox_flush_conflict
...
def _handle_db_error(
    ...
    # 对于数据库错误，使用统一错误码
    try:
        step1_adapter.insert_write_audit(
...
def _handle_conflict(
    ...
    """
    # 读取当前记录状态
    current_record = step1_adapter.get_outbox_by_id(outbox_id)
...
def _process_single_item_inner(
    ...
    if dedup_record:
        ...
        logger.info(f"[outbox:{outbox_id}] Dedupe hit: 已存在成功写入记录 (original_outbox_id={dedup_record.get('outbox_id')})")
    ...
    step1_adapter.renew_lease(outbox_id=outbox_id, worker_id=worker_id)
    ...
    try:
        ...
        result = openmemory_client.StoreResult(
            success=False,
            error=f"connection_error: {e.message}"
        )
    ...
    if result.success:
        ...
        if new_retry_count >= config.max_retries:
            # 超过最大重试：调用 mark_dead
            dead_ok = step1_adapter.mark_dead(
                outbox_id=outbox_id,
                worker_id=worker_id,
                error=error_msg
            )
            ...
            logger.warning(f"[outbox:{outbox_id}] 超过最大重试次数({config.max_retries})，标记为 dead")
            return ProcessResult(
                outbox_id=outbox_id,
                success=False,
                action="reject",
            ...
            retry_ok = step1_adapter.fail_retry(
                outbox_id=outbox_id,
                worker_id=worker_id,
                error=error_msg,
            ...
            logger.info(f"[outbox:{outbox_id}] 重试 {new_retry_count}/{config.max_retries}, 下次尝试: {next_attempt_at.isoformat()}")
            return ProcessResult(
                outbox_id=outbox_id,
                success=False,
                action="redirect",
...
def process_batch(config: WorkerConfig, worker_id: Optional[str] = None) -> list[ProcessResult]:
    ...
    raw_items = step1_adapter.claim_outbox(
        worker_id=worker_id,
    ...
    # 转换为 OutboxItem 对象
    items = [step1_adapter.OutboxItem.from_dict(row) for row in raw_items]
    
    logger.info(f"领取到 {len(items)} 条待处理记录 (worker_id={worker_id}, correlation_id={correlation_id})")
...
def run_loop(config: WorkerConfig, worker_id: Optional[str] = None) -> None:
    ...
    logger.info(f"配置: batch_size={config.batch_size}, max_retries={config.max_retries}, "
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Outbox Worker：处理 logbook.outbox_memory 队列"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=5.0,
        help="loop 模式下每轮间隔秒数 (默认: 5.0)"
    )
    parser.add_argument(
        "--lease-seconds",
        type=int,
        default=120,
        help="Lease 租约有效期秒数 (默认: 120)"
    )
    parser.add_argument(
        "--openmemory-timeout",
        type=float,

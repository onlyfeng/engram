...
def write_reconcile_audit(
    ...
    Args:
        outbox: Outbox 记录字典
        reason: 审计原因（如 outbox_flush_success, outbox_flush_dead, outbox_stale）
    ...
    if outbox.get("retry_count"):
        evidence["retry_count"] = outbox["retry_count"]
    
    if outbox.get("locked_by"):
        evidence["original_locked_by"] = outbox["locked_by"]
    
    if outbox.get("locked_at"):
        evidence["original_locked_at"] = outbox["locked_at"].isoformat() if isinstance(outbox["locked_at"], datetime) else str(outbox["locked_at"])
    
    if extra_evidence:
        evidence["extra"] = extra_evidence
    
    return step1_adapter.insert_write_audit(
...
def reconcile_sent_records(
    ...
    for record in records:
        ...
        has_success = check_audit_exists(conn, outbox_id, "outbox_flush_success")
        has_dedup = check_audit_exists(conn, outbox_id, "outbox_flush_dedup_hit")
...
def reconcile_dead_records(
    ...
    for record in records:
        ...
        has_dead_audit = check_audit_exists(conn, outbox_id, "outbox_flush_dead")
...
def reconcile_stale_records(
    ...
    for record in stale_records:
        ...
        has_stale_audit = check_audit_exists(conn, outbox_id, "outbox_stale")

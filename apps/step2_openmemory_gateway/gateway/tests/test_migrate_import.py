...
验证 engram_step1.migrate 模块可以被正确导入，
确保在容器与本地环境均可执行。

测试覆盖:
- engram_step1.migrate 模块导入
- run_all_checks / run_migrate 函数可调用
- step1_adapter 不再使用 sys.path 注入
...
class TestMigrateModuleImport:
    ...
    def test_import_run_all_checks(self):
        ...
        from engram_step1.migrate import run_all_checks
...
class TestStep1AdapterNoSysPathInjection:
    """验证 step1_adapter 不再使用 sys.path 注入"""

    def test_no_sys_path_manipulation_in_adapter(self):
        """验证 step1_adapter 不包含 sys.path 操作"""
        from gateway import step1_adapter
        import inspect
        
        # 获取模块源代码
        source = inspect.getsource(step1_adapter)
    ...
    def test_adapter_imports_from_engram_step1(self):
        """验证 adapter 从 engram_step1.migrate 导入"""
        from gateway import step1_adapter
        import inspect
        ...
        # 验证从正确的模块导入
        assert "from engram_step1.migrate import run_all_checks, run_migrate" in source

    def test_db_migrate_available_flag(self):
        """测试 _DB_MIGRATE_AVAILABLE 标志正确设置"""
        from gateway.step1_adapter import _DB_MIGRATE_AVAILABLE
        # 如果 engram_step1.migrate 可用，标志应为 True
        # 注意：这依赖于测试环境中 engram_step1 包可用
...
class TestDbMigrateCliBackwardCompatibility:
    ...
    def test_db_migrate_imports_from_migrate_module(self):
        """测试 db_migrate.py 从 engram_step1.migrate 导入"""
        scripts_dir = Path(__file__).parent.parent.parent.parent / "step1_logbook_postgres" / "scripts"
        ...
        # 验证导入语句
        assert "from engram_step1.migrate import" in source
...
class TestGatewayDbCheckPath:
    ...
    def test_check_db_schema_uses_migrate_module(self):
        """测试 check_db_schema 使用 engram_step1.migrate 模块"""
    ...
    def test_run_migration_uses_migrate_module(self):
        """测试 run_migration 使用 engram_step1.migrate 模块"""
        from gateway.step1_adapter import Step1Adapter
    ...
    def test_main_check_step1_db_on_startup(self):
        """测试 main.py 的 check_step1_db_on_startup 函数"""
        ...
        with patch('gateway.main.is_db_migrate_available', return_value=True):
            with patch('gateway.main.ensure_db_ready') as mock_ensure:
                mock_ensure.return_value = Step1DBCheckResult(
                    ok=True,
...
class TestMigrateModuleFunctionSignatures:
    ...
    def test_run_precheck_signature(self):
        ...
        from engram_step1.migrate import run_precheck
        import inspect

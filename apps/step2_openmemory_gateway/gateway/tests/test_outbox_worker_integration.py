...
使用真实 Postgres 数据库（复用 Step1 的 test DB fixture）并 mock OpenMemoryClient.store
覆盖三条路径：
1. 成功写入 (pending -> sent)
...
class TestOutboxWorkerIntegrationSuccess:
    ...
    def test_success_path_status_transition(self, migrated_db, step1_adapter_config):
        ...
        try:
            ...
            with patch("gateway.outbox_worker.openmemory_client.OpenMemoryClient", return_value=mock_client):
                from gateway.outbox_worker import WorkerConfig, process_batch
    ...
    def test_success_path_openmemory_called_with_correct_params(self, migrated_db, step1_adapter_config):
...
class TestOutboxWorkerIntegrationRetry:
    ...
    def test_retry_path_status_and_retry_count(self, migrated_db, step1_adapter_config):
    ...
    def test_retry_path_becomes_dead_after_max_retries(self, migrated_db, step1_adapter_config):
...
class TestOutboxWorkerIntegrationDedup:
    ...
    def test_dedup_path_skips_openmemory_call(self, migrated_db, step1_adapter_config):
    ...
    def test_dedup_path_audit_contains_original_outbox_id(self, migrated_db, step1_adapter_config):
...
class TestOutboxWorkerIntegrationLocking:
    ...
    def test_lock_acquired_during_processing(self, migrated_db, step1_adapter_config):
...
class TestOutboxWorkerIntegrationAuditValidation:
    ...
    def test_audit_action_reason_convention(self, migrated_db, step1_adapter_config):
...
class TestOutboxWorkerIntegrationConflict:
    ...
    def test_conflict_when_lease_stolen_by_second_worker(self, migrated_db, step1_adapter_config):
        """
    ...
    def test_conflict_on_retry_path(self, migrated_db, step1_adapter_config):
        """
    ...
    def test_no_duplicate_audit_on_conflict(self, migrated_db, step1_adapter_config):
        """
...
class TestOutboxWorkerLeaseConflict:
    ...
    def test_lease_expired_conflict_detection(self, migrated_db, step1_adapter_config):
        """
    ...
    def test_sequential_lease_conflict_deterministic(self, migrated_db, step1_adapter_config):
        """
        串行测试 Lease 冲突（确定性场景）
        ...
        try:
            ...
            claimed_items = step1_adapter.claim_outbox(
            ...
            claimed_by_worker2 = step1_adapter.claim_outbox(
            ...
            ack_result = step1_adapter.ack_sent(
                outbox_id=outbox_id,
            ...
            ack_result_2 = step1_adapter.ack_sent(
                outbox_id=outbox_id,
                worker_id="worker-2-seq",
...
class TestOutboxWorkerLeaseRenewal:
    ...
    def test_renew_lease_prevents_reclaim_during_slow_store(self, migrated_db, step1_adapter_config):
        """
        验证续期防止被 reclaim：
    ...
    def test_renew_lease_called_before_and_after_store(self, migrated_db, step1_adapter_config):
        ...
        try:
            ...
            from gateway import step1_adapter
            original_renew = step1_adapter.renew_lease
            ...
            with patch("gateway.outbox_worker.openmemory_client.OpenMemoryClient", return_value=mock_client):
                with patch("gateway.outbox_worker.step1_adapter.renew_lease", side_effect=tracked_renew):
                    from gateway.outbox_worker import WorkerConfig, process_batch
    ...
    def test_second_worker_cannot_reclaim_after_renew(self, migrated_db, step1_adapter_config):
        """
        ...
        try:
            ...
            # Worker 1 续期
            renew_result = step1_adapter.renew_lease(outbox_id, worker_id="worker-1")
            assert renew_result is True
            
            # Worker 2 尝试 claim
            worker2_items = step1_adapter.claim_outbox(
                worker_id="worker-2",
                limit=10,
                lease_seconds=60
            )
            ...
            # Worker 1 仍能成功 ack
            ack_result = step1_adapter.ack_sent(outbox_id, worker_id="worker-1", memory_id="mem_reclaim_003")
    ...
    def test_expired_lease_can_be_reclaimed_without_renew(self, migrated_db, step1_adapter_config):
        """
...
class TestOutboxWorkerSlowStoreTimeout:
    ...
    def test_slow_store_respects_timeout_no_internal_retry(self, migrated_db, step1_adapter_config):
        """
        验证长延迟 store 场景下：
    ...
    def test_timeout_exception_triggers_worker_retry(self, migrated_db, step1_adapter_config):
        """
    ...
    def test_processing_time_with_multiple_retries_controlled(self, migrated_db, step1_adapter_config):
        """
...
class TestOutboxWorkerDatabaseTimeout:
    ...
    def test_db_timeout_triggers_recoverable_path(self, migrated_db, step1_adapter_config, monkeypatch):
        """
        数据库超时触发可恢复路径：
        1. 设置 statement_timeout 为极小值（10ms）
        2. Mock step1_adapter.check_dedup 在执行前添加 pg_sleep 延迟
        ...
        try:
            ...
            # 保存原始函数并替换
            from gateway import step1_adapter as adapter_module
    ...
    def test_outbox_recoverable_after_db_timeout(self, migrated_db, step1_adapter_config, monkeypatch):
        """
    ...
    def test_db_error_classified_correctly(self, migrated_db, step1_adapter_config, monkeypatch):
        """

...
# 确保 engram_step1 可导入
step1_scripts_path = os.path.join(
    os.path.dirname(__file__), 
    "..", "..", "..", "..", 
    "step1_logbook_postgres", "scripts"
)
if step1_scripts_path not in sys.path:
    sys.path.insert(0, os.path.abspath(step1_scripts_path))
...
class TestGatewayErrorCodes:
    ...
    async def test_openmemory_connection_error_reason(self, mock_config):
        ...
        with patch("gateway.main.get_config") as mock_get_config, \
            ...
            assert audit["reason"] == ErrorCode.OPENMEMORY_WRITE_FAILED_CONNECTION, \
                f"OpenMemory 连接失败时 reason 必须为 {ErrorCode.OPENMEMORY_WRITE_FAILED_CONNECTION}，实际: {audit['reason']}"

    @pytest.mark.asyncio
    async def test_actor_unknown_reject_reason(self, mock_config):
        """关键路径测试: Actor 不存在且策略为 reject -> reason 必须为 actor_unknown:reject"""
        from gateway.main import _validate_actor_user
...
class TestErrorCodeConsistency:
    ...
    def test_gateway_and_worker_use_same_error_codes(self):
        ...
        # 导入两个模块的 ErrorCode
        from gateway.main import ErrorCode as GatewayErrorCode
        from gateway.outbox_worker import ErrorCode as WorkerErrorCode

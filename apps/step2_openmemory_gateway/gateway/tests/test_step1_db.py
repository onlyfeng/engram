...
class TestGetDbSingleton:
    ...
    def setup_method(self):
        """每个测试前重置状态"""
        # 导入并重置模块状态
        from gateway import step1_db
        step1_db._db_instances.clear()
        step1_db._default_dsn = None
    ...
    def test_same_dsn_returns_same_instance(self):
        """相同 DSN 返回相同实例"""
        from gateway.step1_db import get_db, reset_db, _db_instances
        
        # Mock Step1Database 以避免实际连接
        with patch('gateway.step1_db.Step1Database') as MockDB:
    ...
    def test_different_dsn_returns_different_instance(self):
        """不同 DSN 返回不同实例"""
        from gateway.step1_db import get_db, _db_instances
    ...
    def test_reset_db_clears_all_instances(self):
        ...
        with patch('gateway.step1_db.Step1Database') as MockDB:
            ...
            from gateway.step1_db import _db_instances as instances_after
    ...
    def test_reset_db_with_specific_dsn(self):
        ...
        with patch('gateway.step1_db.Step1Database') as MockDB:
            ...
            step1_db.get_db(dsn=dsn1)
            step1_db.get_db(dsn=dsn2)
            
            assert len(step1_db._db_instances) == 2
            
            step1_db.reset_db(dsn=dsn1)
            
            assert dsn1 not in step1_db._db_instances
            assert dsn2 in step1_db._db_instances
    ...
    def test_set_default_dsn(self):
        ...
        with patch('gateway.step1_db.Step1Database') as MockDB:
            ...
            step1_db.set_default_dsn(dsn)
            
            assert step1_db._default_dsn == dsn
    ...
    def test_get_db_uses_default_dsn_when_no_param(self):
        ...
        with patch('gateway.step1_db.Step1Database') as MockDB:
            ...
            # 先用指定 DSN 创建实例
            step1_db.get_db(dsn=dsn)
            
            # 无参数调用应该使用默认 DSN
            db = step1_db.get_db()
            
            # 应该返回同一个实例（因为 DSN 相同）
            assert dsn in step1_db._db_instances
    ...
    def test_config_dsn_overrides_env_var(self):
        ...
        with patch('gateway.step1_db.Step1Database') as MockDB:
            ...
            with patch.dict(os.environ, {"POSTGRES_DSN": env_dsn}):
                # 使用环境变量的 DSN
                db_env = step1_db.get_db()
                
                # 使用配置的 DSN（应该创建不同实例）
                db_config = step1_db.get_db(dsn=config_dsn)
...
class TestGatewayConfigIntegration:
    ...
    def test_gateway_uses_config_dsn(self):
        ...
        from gateway import step1_db
        from gateway.config import get_config, reset_config
        ...
        with patch('gateway.step1_db.Step1Database') as MockDB:
            ...
            with patch.dict(os.environ, env_vars, clear=False):
                ...
                step1_db.set_default_dsn(config_dsn)
                step1_db.get_db(dsn=config_dsn)
                
                # 确认使用了 config_dsn
                assert config_dsn in step1_db._db_instances
    ...
    def test_different_config_dsn_creates_separate_db(self):
        ...
        with patch('gateway.step1_db.Step1Database') as MockDB:
            ...
            # 模拟两个不同配置场景
            db1 = step1_db.get_db(dsn=dsn1)
            db2 = step1_db.get_db(dsn=dsn2)
            ...
            # 验证缓存中有两个条目
            assert len(step1_db._db_instances) == 2
            assert dsn1 in step1_db._db_instances
            assert dsn2 in step1_db._db_instances


class TestStep1DBCheck:
    """Step1 DB 检查功能测试"""

    def setup_method(self):
        """每个测试前重置状态"""
        from gateway import step1_adapter
        step1_adapter._adapter_instance = None
    ...
    def test_check_db_schema_success(self):
        """测试 DB 检查成功的场景"""
        from gateway.step1_adapter import (
            Step1Adapter,
            Step1DBCheckResult,
            check_db_schema,
        )
        ...
        with patch('gateway.step1_adapter.run_all_checks') as mock_check:
            ...
            with patch('gateway.step1_adapter.get_connection') as mock_conn:
                mock_conn_instance = MagicMock()
                mock_conn.return_value = mock_conn_instance
                
                with patch('gateway.step1_adapter._DB_MIGRATE_AVAILABLE', True):
                    adapter = Step1Adapter(dsn="postgresql://test@localhost/test")
    ...
    def test_check_db_schema_missing_items(self):
        """测试 DB 检查发现缺失项的场景"""
        from gateway.step1_adapter import Step1Adapter, Step1DBCheckResult
    ...
    def test_check_db_schema_module_not_available(self):
        ...
        from gateway.step1_adapter import Step1Adapter
        
        with patch('gateway.step1_adapter._DB_MIGRATE_AVAILABLE', False):
            adapter = Step1Adapter(dsn="postgresql://test@localhost/test")
            result = adapter.check_db_schema()
    ...
    def test_ensure_db_ready_auto_migrate_success(self):
        ...
        with patch('gateway.step1_adapter.run_all_checks', side_effect=mock_check_side_effect):
            with patch('gateway.step1_adapter.run_migrate') as mock_migrate:
    ...
    def test_ensure_db_ready_no_auto_migrate_raises_error(self):
        """测试不自动迁移时抛出错误"""
        from gateway.step1_adapter import Step1Adapter, Step1DBCheckError
        ...
        with patch('gateway.step1_adapter.run_all_checks') as mock_check:
            ...
            with patch('gateway.step1_adapter.get_connection') as mock_conn:
                ...
                with patch('gateway.step1_adapter._DB_MIGRATE_AVAILABLE', True):
                    ...
                    with pytest.raises(Step1DBCheckError) as exc_info:
...
class TestCheckStep1DbOnStartup:
    """测试 main.py 中的 check_step1_db_on_startup 函数"""
    ...
    def test_skip_check_when_disabled(self):
        """测试禁用检查时跳过"""
        from gateway.main import check_step1_db_on_startup
        ...
        config = GatewayConfig(
            ...
            openmemory_base_url="http://localhost:8080",
            step1_check_on_startup=False,
        )
        
        result = check_step1_db_on_startup(config)
    ...
    def test_check_success(self):
        ...
        from gateway.main import check_step1_db_on_startup
        from gateway.config import GatewayConfig
        from gateway.step1_adapter import Step1DBCheckResult
        ...
        with patch('gateway.main.is_db_migrate_available', return_value=True):
            with patch('gateway.main.ensure_db_ready') as mock_ensure:
                mock_ensure.return_value = Step1DBCheckResult(ok=True)
    ...
    def test_check_failure_with_auto_migrate_disabled(self):
        ...
        with patch('gateway.main.is_db_migrate_available', return_value=True):
            with patch('gateway.main.ensure_db_ready') as mock_ensure:
                mock_ensure.side_effect = Step1DBCheckError(
                    message="DB 结构不完整",

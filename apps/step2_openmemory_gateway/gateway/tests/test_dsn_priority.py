...
验证 Step1Adapter 的 DSN 选择逻辑:
- 显式传入 dsn 参数时使用强覆盖策略
- DSN 优先级: 显式参数 > POSTGRES_DSN > TEST_PG_DSN

此测试确保 step1_adapter 与 step1_db 的 DSN 设置逻辑对齐，
保证"单一事实来源"的 DSN 传递。
"""
...
class TestDSNPriority:
    ...
    def test_explicit_dsn_overrides_env(self):
        ...
        try:
            ...
            # 创建适配器时显式传入 dsn
            adapter = step1_adapter.Step1Adapter(dsn=explicit_dsn)
    ...
    def test_none_dsn_preserves_env(self):
        ...
        try:
            # 创建适配器时不传入 dsn
            adapter = step1_adapter.Step1Adapter(dsn=None)
            
            # 断言: 环境变量应该保持不变
            assert os.environ["POSTGRES_DSN"] == existing_dsn, (
                "不传入 dsn 参数时不应修改环境变量"
            )
    ...
    def test_get_adapter_with_explicit_dsn(self):
        """
        get_adapter(dsn) 应该正确传递 dsn 到 Step1Adapter
        """
        from gateway import step1_adapter
        step1_adapter.reset_adapter()
        ...
        try:
            ...
            adapter = step1_adapter.get_adapter(dsn=explicit_dsn)
    ...
    def test_dsn_priority_docstring_alignment(self):
        """
        验证 step1_adapter 与 step1_db 的 DSN 优先级一致
        
        两个模块应该遵循相同的优先级: 显式参数 > POSTGRES_DSN > TEST_PG_DSN
        """
        from gateway import step1_adapter
        
        # 检查 Step1Adapter.__init__ 的 docstring 包含正确的优先级描述
        docstring = step1_adapter.Step1Adapter.__init__.__doc__
...
class TestDSNOverrideWithMock:
    ...
    def test_multiple_adapters_last_dsn_wins(self):
        ...
        try:
            ...
            adapter1 = step1_adapter.Step1Adapter(dsn=dsn1)
            assert os.environ["POSTGRES_DSN"] == dsn1
            
            # 创建第二个适配器
            dsn2 = "postgresql://second:second@localhost/second_db"
            adapter2 = step1_adapter.Step1Adapter(dsn=dsn2)

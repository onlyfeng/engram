-- ============================================================
-- Engram Step1: 角色与权限管理脚本
-- ============================================================
--
-- 本脚本定义数据库角色及其在各 schema 上的权限，支持多租户隔离。
--
-- 角色设计（NOLOGIN 权限角色）：
--   Engram Step1 角色（用于 Engram 专属 schema）：
...
--   Step1 服务账号：
--     step1_migrator           - 迁移账号，继承 engram_migrator
--     step1_svc                - 运行账号，继承 engram_app_readwrite
--
--   OpenMemory 服务账号：
...
-- 服务账号映射：
--   Step1 服务账号：
--     step1_migrator           -> engram_migrator      (DDL)
--     step1_svc                -> engram_app_readwrite (DML)
--
--   OpenMemory 服务账号：
--     openmemory_migrator_login -> openmemory_migrator  (DDL)
--     openmemory_svc            -> openmemory_app       (DML)

-- Step1 服务账号 membership
DO $$
BEGIN
    -- step1_migrator -> engram_migrator
    IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'step1_migrator') THEN
        EXECUTE 'GRANT engram_migrator TO step1_migrator';
        RAISE NOTICE 'Granted engram_migrator TO step1_migrator';
    END IF;
    
    -- step1_svc -> engram_app_readwrite
    IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'step1_svc') THEN
        EXECUTE 'GRANT engram_app_readwrite TO step1_svc';
        RAISE NOTICE 'Granted engram_app_readwrite TO step1_svc';
    END IF;
END $$;
...
--   登录角色（如 step1_migrator）执行迁移时需要先 SET ROLE engram_migrator，
...
-- 登录角色（如 step1_migrator）通过 SET ROLE engram_migrator 执行 DDL

...
BEGIN
    ...
    FOREACH role_name IN ARRAY expected_roles LOOP
        ...
        IF role_count = 0 THEN
            ...
            RAISE NOTICE '  remedy: 执行 04_roles_and_grants.sql 或 06_step3_roles_and_grants.sql 创建角色';
...
DECLARE
    ...
    login_mappings TEXT[][] := ARRAY[
        ARRAY['step1_migrator', 'engram_migrator', 'Step1 迁移账号'],
        ARRAY['step1_svc', 'engram_app_readwrite', 'Step1 运行账号'],
        ARRAY['openmemory_migrator_login', 'openmemory_migrator', 'OpenMemory 迁移账号'],
        ARRAY['openmemory_svc', 'openmemory_app', 'OpenMemory 运行账号'],
        ARRAY['step3_migrator_login', 'step3_migrator', 'Step3 迁移账号'],
        ARRAY['step3_svc', 'step3_app', 'Step3 运行账号']
    ];
    v_mapping TEXT[];
    v_login_role TEXT;
    v_target_role TEXT;
...
BEGIN
    ...
    -- step3_migrator 不应有 public CREATE 权限（使用独立 step3 schema）
    IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'step3_migrator') THEN
        SELECT pg_catalog.has_schema_privilege('step3_migrator', 'public', 'CREATE') INTO can_create;
        IF can_create THEN
            RAISE WARNING 'WARN: step3_migrator 有 public schema 的 CREATE 权限（应使用独立 step3 schema）';
            RAISE NOTICE '  remedy: REVOKE CREATE ON SCHEMA public FROM step3_migrator;';
        ELSE
            RAISE NOTICE 'OK: step3_migrator 无 public schema CREATE 权限';
        END IF;
    END IF;
...
-- 4.5 验证 step3 schema 存在性、owner 和权限
DO $$
DECLARE
    v_schema_exists BOOLEAN;
    v_schema_owner TEXT;
...
BEGIN
    RAISE NOTICE '';
    RAISE NOTICE '=== 4.5 Step3 schema 验证 ===';
    
    -- 检查 schema 是否存在
    SELECT EXISTS(SELECT 1 FROM information_schema.schemata WHERE schema_name = 'step3') INTO v_schema_exists;
    
    IF NOT v_schema_exists THEN
        RAISE WARNING 'FAIL: step3 schema 不存在';
        RAISE NOTICE '  remedy: 执行 06_step3_roles_and_grants.sql';
        RAISE NOTICE '后续权限验证跳过，请先创建 step3 schema';
        RETURN;
    END IF;
    
    RAISE NOTICE 'OK: step3 schema 存在';
    
    -- 验证 schema owner = step3_migrator
    SELECT nspowner::regrole::text INTO v_schema_owner
    FROM pg_namespace
    WHERE nspname = 'step3';
    
    IF v_schema_owner = 'step3_migrator' THEN
        RAISE NOTICE 'OK: step3 schema owner = step3_migrator';
    ELSE
        RAISE WARNING 'FAIL: step3 schema owner = % (预期: step3_migrator)', v_schema_owner;
        RAISE NOTICE '  remedy: ALTER SCHEMA step3 OWNER TO step3_migrator;';
        v_fail_count := v_fail_count + 1;
    END IF;
    
    -- step3_migrator 应有 CREATE 权限
    SELECT pg_catalog.has_schema_privilege('step3_migrator', 'step3', 'CREATE') INTO can_create;
    IF can_create THEN
        RAISE NOTICE 'OK: step3_migrator 有 step3 schema CREATE 权限';
    ELSE
        RAISE WARNING 'FAIL: step3_migrator 无 step3 schema CREATE 权限';
        RAISE NOTICE '  remedy: GRANT ALL PRIVILEGES ON SCHEMA step3 TO step3_migrator;';
        v_fail_count := v_fail_count + 1;
    END IF;
    
    -- step3_app 应有 USAGE 权限
    SELECT pg_catalog.has_schema_privilege('step3_app', 'step3', 'USAGE') INTO can_usage;
    IF can_usage THEN
        RAISE NOTICE 'OK: step3_app 有 step3 schema USAGE 权限';
    ELSE
        RAISE WARNING 'FAIL: step3_app 无 step3 schema USAGE 权限';
        RAISE NOTICE '  remedy: GRANT USAGE ON SCHEMA step3 TO step3_app;';
        v_fail_count := v_fail_count + 1;
    END IF;
    
    -- step3_app 不应有 CREATE 权限
    SELECT pg_catalog.has_schema_privilege('step3_app', 'step3', 'CREATE') INTO can_create;
    IF can_create THEN
        RAISE WARNING 'FAIL: step3_app 有 step3 schema CREATE 权限（应仅限 DML）';
        RAISE NOTICE '  remedy: REVOKE CREATE ON SCHEMA step3 FROM step3_app;';
        v_fail_count := v_fail_count + 1;
    ELSE
        RAISE NOTICE 'OK: step3_app 无 step3 schema CREATE 权限';
    END IF;
    
    -- 汇总
    IF v_fail_count > 0 THEN
        RAISE NOTICE 'Step3 schema 验证: % 项 FAIL', v_fail_count;
    ELSE
        RAISE NOTICE 'Step3 schema 验证: 全部通过';
    END IF;
END $$;

-- 4.6 验证 step3 表级 DML 权限（如果表存在）
DO $$
DECLARE
    v_schema_exists BOOLEAN;
    v_table_count INT;
...
BEGIN
    RAISE NOTICE '';
    RAISE NOTICE '=== 4.6 Step3 表级 DML 权限验证 ===';
    
    -- 检查 schema 是否存在
    SELECT EXISTS(SELECT 1 FROM information_schema.schemata WHERE schema_name = 'step3') INTO v_schema_exists;
    
    IF NOT v_schema_exists THEN
        RAISE NOTICE 'SKIP: step3 schema 不存在，跳过表级权限验证';
        RETURN;
    END IF;
    ...
    WHERE table_schema = 'step3' AND table_type = 'BASE TABLE';
    
    IF v_table_count = 0 THEN
        RAISE NOTICE 'SKIP: step3 schema 中无表，跳过表级权限验证';
        RETURN;
    END IF;
    ...
    FOR v_table_name IN 
        ...
        WHERE table_schema = 'step3' AND table_type = 'BASE TABLE'
    LOOP
        -- 检查 step3_app 的 DML 权限
        SELECT 
            has_table_privilege('step3_app', 'step3.' || v_table_name, 'SELECT'),
            has_table_privilege('step3_app', 'step3.' || v_table_name, 'INSERT'),
            has_table_privilege('step3_app', 'step3.' || v_table_name, 'UPDATE'),
            has_table_privilege('step3_app', 'step3.' || v_table_name, 'DELETE'),
            has_table_privilege('step3_app', 'step3.' || v_table_name, 'TRUNCATE')
        INTO v_has_select, v_has_insert, v_has_update, v_has_delete, v_has_truncate;
        
        -- step3_app 应有 SELECT/INSERT/UPDATE/DELETE，不应有 TRUNCATE
        IF v_has_select AND v_has_insert AND v_has_update AND v_has_delete THEN
            RAISE NOTICE 'OK: step3_app 对 step3.% 有 SELECT/INSERT/UPDATE/DELETE 权限', v_table_name;
        ELSE
            RAISE WARNING 'FAIL: step3_app 对 step3.% 缺少 DML 权限 (S=%,I=%,U=%,D=%)', 
                v_table_name, v_has_select, v_has_insert, v_has_update, v_has_delete;
            RAISE NOTICE '  remedy: GRANT SELECT, INSERT, UPDATE, DELETE ON step3.% TO step3_app;', v_table_name;
            v_fail_count := v_fail_count + 1;
        END IF;
        
        -- step3_app 不应有 TRUNCATE 权限（安全要求）
        IF v_has_truncate THEN
            RAISE WARNING 'WARN: step3_app 对 step3.% 有 TRUNCATE 权限（应仅限 migrator）', v_table_name;
        END IF;
        
        -- 检查 step3_migrator 的完整权限
        SELECT 
            has_table_privilege('step3_migrator', 'step3.' || v_table_name, 'SELECT'),
            has_table_privilege('step3_migrator', 'step3.' || v_table_name, 'INSERT'),
            has_table_privilege('step3_migrator', 'step3.' || v_table_name, 'UPDATE'),
            has_table_privilege('step3_migrator', 'step3.' || v_table_name, 'DELETE'),
            has_table_privilege('step3_migrator', 'step3.' || v_table_name, 'TRUNCATE')
        INTO v_has_select, v_has_insert, v_has_update, v_has_delete, v_has_truncate;
        
        IF v_has_select AND v_has_insert AND v_has_update AND v_has_delete AND v_has_truncate THEN
            RAISE NOTICE 'OK: step3_migrator 对 step3.% 有完整权限', v_table_name;
        ELSE
            RAISE WARNING 'FAIL: step3_migrator 对 step3.% 缺少权限', v_table_name;
            RAISE NOTICE '  remedy: GRANT ALL PRIVILEGES ON step3.% TO step3_migrator;', v_table_name;
            v_fail_count := v_fail_count + 1;
        END IF;
    END LOOP;
    ...
    -- 汇总
    IF v_fail_count > 0 THEN
        RAISE NOTICE 'Step3 表级权限验证: % 项 FAIL', v_fail_count;
    ELSE
        RAISE NOTICE 'Step3 表级权限验证: 全部通过';
    END IF;
END $$;

-- 4.7 验证 step3 序列权限（如果序列存在）
DO $$
DECLARE
    v_schema_exists BOOLEAN;
    v_seq_count INT;
...
BEGIN
    RAISE NOTICE '';
    RAISE NOTICE '=== 4.7 Step3 序列权限验证 ===';
    
    -- 检查 schema 是否存在
    SELECT EXISTS(SELECT 1 FROM information_schema.schemata WHERE schema_name = 'step3') INTO v_schema_exists;
    
    IF NOT v_schema_exists THEN
        RAISE NOTICE 'SKIP: step3 schema 不存在，跳过序列权限验证';
        RETURN;
    END IF;
    ...
    WHERE sequence_schema = 'step3';
    
    IF v_seq_count = 0 THEN
        RAISE NOTICE 'SKIP: step3 schema 中无序列，跳过序列权限验证';
        RETURN;
    END IF;
    ...
    FOR v_seq_name IN 
        ...
        WHERE sequence_schema = 'step3'
    LOOP
        -- 检查 step3_app 的 USAGE 权限
        SELECT has_sequence_privilege('step3_app', 'step3.' || v_seq_name, 'USAGE')
        INTO v_has_usage;
        
        IF v_has_usage THEN
            RAISE NOTICE 'OK: step3_app 对 step3.% 有 USAGE 权限', v_seq_name;
        ELSE
            RAISE WARNING 'FAIL: step3_app 对 step3.% 缺少 USAGE 权限', v_seq_name;
            RAISE NOTICE '  remedy: GRANT USAGE ON SEQUENCE step3.% TO step3_app;', v_seq_name;
            v_fail_count := v_fail_count + 1;
        END IF;
    END LOOP;
    ...
    -- 汇总
    IF v_fail_count > 0 THEN
        RAISE NOTICE 'Step3 序列权限验证: % 项 FAIL', v_fail_count;
    ELSE
        RAISE NOTICE 'Step3 序列权限验证: 全部通过';
    END IF;
END $$;
...
-- 检查 step3_migrator 对 step3_app 的 TABLE/SEQUENCE 默认授权
DO $$
DECLARE
    v_schema TEXT;
    v_fail_count INT := 0;
...
BEGIN
    ...
    -- 5.2 验证 step3_migrator -> step3_app 默认权限
    RAISE NOTICE '';
    RAISE NOTICE '--- 5.2 Step3 默认权限 ---';
    
    -- 获取 step3 schema OID
    SELECT oid INTO v_schema_oid FROM pg_namespace WHERE nspname = 'step3';
    
    IF v_schema_oid IS NULL THEN
        RAISE NOTICE 'SKIP: step3 schema 不存在，跳过默认权限验证';
    ELSE
        -- 获取角色 OID
        SELECT oid INTO v_grantor_oid FROM pg_roles WHERE rolname = 'step3_migrator';
        SELECT oid INTO v_grantee_oid FROM pg_roles WHERE rolname = 'step3_app';
        
        IF v_grantor_oid IS NULL THEN
            RAISE WARNING 'FAIL: step3_migrator 角色不存在';
            RAISE NOTICE '  remedy: 执行 06_step3_roles_and_grants.sql';
            v_fail_count := v_fail_count + 1;
        ELSIF v_grantee_oid IS NULL THEN
            RAISE WARNING 'FAIL: step3_app 角色不存在';
            RAISE NOTICE '  remedy: 执行 06_step3_roles_and_grants.sql';
            v_fail_count := v_fail_count + 1;
        ELSE
            -- 检查 TABLE 默认权限 (defaclobjtype = 'r')
            ...
            IF v_count > 0 THEN
                RAISE NOTICE 'OK: step3_migrator 在 step3 对 step3_app 有 TABLE 默认授权';
            ELSE
                RAISE WARNING 'FAIL: step3_migrator 在 step3 对 step3_app 无 TABLE 默认授权';
                RAISE NOTICE '  remedy: ALTER DEFAULT PRIVILEGES FOR ROLE step3_migrator IN SCHEMA step3 GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO step3_app;';
                v_fail_count := v_fail_count + 1;
            END IF;
            ...
            IF v_count > 0 THEN
                RAISE NOTICE 'OK: step3_migrator 在 step3 对 step3_app 有 SEQUENCE 默认授权';
            ELSE
                RAISE WARNING 'FAIL: step3_migrator 在 step3 对 step3_app 无 SEQUENCE 默认授权';
                RAISE NOTICE '  remedy: ALTER DEFAULT PRIVILEGES FOR ROLE step3_migrator IN SCHEMA step3 GRANT USAGE ON SEQUENCES TO step3_app;';
                v_fail_count := v_fail_count + 1;
            END IF;
        END IF;
    END IF;
...
DECLARE
    ...
    db_priv_checks TEXT[][] := ARRAY[
        ...
        -- Step3 角色
        ARRAY['step3_migrator', 'Y', 'Y', 'Y'],           -- 迁移角色需要 CREATE/TEMP
        ARRAY['step3_app', 'Y', 'N', 'N'],                -- 应用角色无需 CREATE/TEMP
        -- LOGIN 角色（如存在）
        ARRAY['step1_migrator', 'Y', 'Y', 'Y'],           -- 继承 engram_migrator
        ARRAY['step1_svc', 'Y', 'N', 'N'],                -- 继承 engram_app_readwrite
        ARRAY['openmemory_migrator_login', 'Y', 'Y', 'Y'],-- 继承 openmemory_migrator
        ARRAY['openmemory_svc', 'Y', 'N', 'N'],           -- 继承 openmemory_app
        ARRAY['step3_migrator_login', 'Y', 'Y', 'Y'],     -- 继承 step3_migrator
        ARRAY['step3_svc', 'Y', 'N', 'N']                 -- 继承 step3_app
    ];
    v_check TEXT[];
    v_role TEXT;
...
BEGIN
    ...
    FOREACH v_check SLICE 1 IN ARRAY db_priv_checks LOOP
        ...
        IF NOT v_role_exists THEN
            ...
            IF v_role IN ('step1_migrator', 'step1_svc', 'openmemory_migrator_login', 'openmemory_svc', 'step3_migrator_login', 'step3_svc') THEN
                -- 跳过不存在的 LOGIN 角色（静默）
                CONTINUE;
            ELSE
        ...
        -- 输出详细状态（仅对重要角色）
        IF v_role IN ('engram_migrator', 'engram_app_readwrite', 'openmemory_migrator', 'openmemory_app', 'step3_migrator', 'step3_app') THEN
...
BEGIN
    RAISE NOTICE '';
    RAISE NOTICE '=== 8. step1_migrator 默认权限验证 ===';
    
    -- 检查 step1_migrator 角色是否存在
    SELECT EXISTS(SELECT 1 FROM pg_roles WHERE rolname = 'step1_migrator') INTO v_grantor_exists;
    IF NOT v_grantor_exists THEN
        RAISE NOTICE 'SKIP: step1_migrator 角色不存在（可能未执行 00_init_service_accounts.sh）';
        RETURN;
    END IF;
    
    RAISE NOTICE 'step1_migrator 角色存在，检查默认权限...';
    ...
    FOREACH v_schema IN ARRAY engram_schemas LOOP
        ...
        -- 检查 step1_migrator 是否为该 schema 设置了默认权限
        ...
        WHERE n.nspname = v_schema
          AND r.rolname = 'step1_migrator';
        
        IF v_defacl_count > 0 THEN
            RAISE NOTICE 'OK: step1_migrator 在 % schema 有 % 条默认权限配置', v_schema, v_defacl_count;
        ELSE
            RAISE WARNING 'WARN: step1_migrator 在 % schema 无默认权限配置（应使用 engram_migrator）', v_schema;
            v_fail_count := v_fail_count + 1;
        END IF;
    END LOOP;
    ...
    -- 汇总
    IF v_fail_count > 0 THEN
        RAISE NOTICE 'step1_migrator 默认权限验证: % 个 schema 无配置（正常，使用 engram_migrator 设置）', v_fail_count;
    ELSE
        RAISE NOTICE 'step1_migrator 默认权限验证: 全部通过';
    END IF;
END $$;
...
BEGIN
    ...
    -- 9.3 列出 step3_migrator 的所有默认权限
    RAISE NOTICE '';
    RAISE NOTICE '--- 9.3 step3_migrator 默认权限 ---';
    
    SELECT EXISTS(SELECT 1 FROM pg_roles WHERE rolname = 'step3_migrator') INTO v_grantor_exists;
    IF NOT v_grantor_exists THEN
        RAISE NOTICE 'SKIP: step3_migrator 角色不存在';
    ELSE
        FOR v_rec IN
            SELECT 
                n.nspname AS schema_name,
            ...
            JOIN pg_roles r ON r.oid = da.defaclrole
            CROSS JOIN LATERAL aclexplode(da.defaclacl) AS a
            WHERE r.rolname = 'step3_migrator'
              AND n.nspname = 'step3'
            GROUP BY n.nspname, da.defaclobjtype
            ORDER BY n.nspname, da.defaclobjtype
        LOOP
...
BEGIN
    ...
    -- 检查 step1_migrator 是否存在
    SELECT EXISTS(SELECT 1 FROM pg_roles WHERE rolname = 'step1_migrator') INTO v_migrator_exists;
    ...
    RAISE NOTICE '  - step1_migrator 存在 = %', v_migrator_exists;
    IF v_schema_exists THEN
        RAISE NOTICE '  - % schema owner = %', v_schema, v_schema_owner;
    END IF;
    RAISE NOTICE '';
    RAISE NOTICE '核心验证预期：';
    RAISE NOTICE '  1. NOLOGIN 角色存在（engram_*, openmemory_*, step3_*）';
    RAISE NOTICE '  2. LOGIN 角色正确继承对应 NOLOGIN 角色';
    RAISE NOTICE '     - step1_migrator -> engram_migrator';
    RAISE NOTICE '     - step1_svc -> engram_app_readwrite';
    RAISE NOTICE '     - openmemory_migrator_login -> openmemory_migrator';
    RAISE NOTICE '     - openmemory_svc -> openmemory_app';
    RAISE NOTICE '     - step3_migrator_login -> step3_migrator';
    RAISE NOTICE '     - step3_svc -> step3_app';
    ...
    RAISE NOTICE '  7. step3 schema 存在且 owner=step3_migrator';
    RAISE NOTICE '  8. step3_migrator 在 step3 有 CREATE 权限（含 CREATE INDEX）';
    RAISE NOTICE '  9. step3_app 在 step3 有 USAGE 且无 CREATE 权限';
    RAISE NOTICE '  10. step3_app 对表有 SELECT/INSERT/UPDATE/DELETE（无 TRUNCATE）';
    RAISE NOTICE '  11. step3_app 对序列有 USAGE 权限';
    ...
    IF v_migrator_exists THEN
        RAISE NOTICE '状态: step1_migrator 已创建，默认权限应已配置';
    ELSE
        RAISE NOTICE '状态: step1_migrator 未创建，请先执行 00_init_service_accounts.sh';
    END IF;

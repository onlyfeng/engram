...
使用方法:
    python -m engram_step1.cli.db_migrate
    python -m engram_step1.cli.db_migrate --config /path/to/config.toml
    python -m engram_step1.cli.db_migrate --apply-roles                    # 执行角色权限脚本（需要 admin/superuser）
    python -m engram_step1.cli.db_migrate --apply-openmemory-grants        # 执行 OpenMemory schema 权限脚本
    python -m engram_step1.cli.db_migrate --verify                         # 执行权限验证脚本（99）

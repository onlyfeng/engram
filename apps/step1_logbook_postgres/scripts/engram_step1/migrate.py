...
# SQL 文件前缀分类
# 默认执行：结构性 DDL 脚本
DDL_SCRIPT_PREFIXES = {"01", "02", "03", "06", "07", "08"}
...
def get_repair_commands_hint(error_code: str = None, target_db: str = None) -> dict:
    ...
    base_commands = {
        "bootstrap": "python step1_logbook_postgres/scripts/db_bootstrap.py",
        "migrate": "python step1_logbook_postgres/scripts/db_migrate.py",
        "migrate_with_roles": "python step1_logbook_postgres/scripts/db_migrate.py --apply-roles --apply-openmemory-grants",
        "verify": "python step1_logbook_postgres/scripts/db_migrate.py --verify",
        "docker_bootstrap": "docker compose -f docker-compose.unified.yml up bootstrap_roles",
        "docker_migrate": "docker compose -f docker-compose.unified.yml up step1_migrate openmemory_migrate",
    }
    ...
    if error_code in ("SCHEMA_MISSING", "TABLE_MISSING", "COLUMN_MISSING", "INDEX_MISSING", "TRIGGER_MISSING", "MATVIEW_MISSING"):
        ...
        return {
            ...
            "recommended_commands": [
                "# 执行 OpenMemory 权限脚本",
                "python step1_logbook_postgres/scripts/db_migrate.py --apply-openmemory-grants",
                "",
                "# 或完整初始化",
                base_commands["bootstrap"],
                "",
        ...
        return {
            ...
            "recommended_commands": [
                ...
                "",
                "# 重新运行预检",
                "python step1_logbook_postgres/scripts/db_migrate.py --precheck-only",
            ],
        }
...
def run_migrate(
    ...
    try:
        ...
        # migrate.py 在 engram_step1/ 目录下，sql 在 ../sql/
        ...
        # 验证必需的 01_step1_schema.sql 存在
        schema_sql = sql_dir / "01_step1_schema.sql"

#!/usr/bin/env python3
"""
render_views.py - 生成 logbook 的视图文件
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        prog="render_views",
        description="生成 logbook 的视图文件（manifest.csv, index.md）",
    )

    parser.add_argument(
        "--out-dir", "-o",
        default=DEFAULT_OUT_DIR,
        help=f"输出目录 (default: {DEFAULT_OUT_DIR})",
    )
    parser.add_argument(
        "--limit", "-n",
        type=int,
        default=DEFAULT_LIMIT,
        help=f"最近条目数量上限 (default: {DEFAULT_LIMIT})",
    )
    parser.add_argument(
        "--item-type", "-t",
        help="按 item_type 筛选",

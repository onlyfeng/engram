...
class TestDryRunMode:
    ...
    def test_dry_run_reports_pending_status(
        self, source_store, target_store, sample_files
    ):
...
class TestVerifyMode:
    ...
    def test_skip_existing_with_matching_hash(
        self, source_store, target_store, sample_files
    ):
        """测试跳过哈希匹配的已存在文件"""
        # 先迁移
...
class TestFailureHandling:
    ...
    def test_records_errors_in_result(
        self, source_store, target_store, sample_files
    ):
        """测试错误被记录到结果中"""
    ...
    def test_continues_after_failure(
        self, source_store, temp_target_dir, sample_files
    ):
        """测试单个文件失败后继续处理其他文件"""
        target_store = LocalArtifactsStore(root=temp_target_dir)
...
class TestDbUpdate:
    ...
    def test_update_db_uris_executes_correct_sql(
    ...
    def test_update_db_skips_failed_items(
        self, source_store, target_store
    ):
        """测试数据库更新跳过失败的项"""
        migrator = ArtifactMigrator(
    ...
    def test_update_db_rollback_on_error(
        self, source_store, target_store
    ):
        """测试数据库更新失败时回滚"""
        migrator = ArtifactMigrator(
    ...
    def test_dry_run_skips_db_update(
        self, source_store, target_store
    ):
...
class TestLimit:
    ...
    def test_limit_restricts_file_count(
        self, source_store, target_store, sample_files
    ):
        """测试 limit 参数限制文件数量"""
        migrator = ArtifactMigrator(
...
class TestDeleteSource:
    ...
    def test_delete_source_after_migration(
        self, temp_source_dir, temp_target_dir, sample_files
    ):
        """测试迁移后删除源文件"""
        source_store = LocalArtifactsStore(root=temp_source_dir)
        target_store = LocalArtifactsStore(root=temp_target_dir)
    ...
    def test_trash_prefix_moves_to_trash(
        self, temp_source_dir, temp_target_dir, sample_files
    ):
...
class TestFactoryFunctions:
    ...
    def test_run_migration_convenience(
        self, temp_source_dir, temp_target_dir, sample_files
    ):

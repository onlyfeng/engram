...
- step1_cli: scm ensure-repo, sync-svn, sync-gitlab-commits, sync-gitlab-mrs, sync-gitlab-reviews
"""
...
class TestLogbookCliContractShape:
    ...
    def test_create_item_success_shape(self, migrated_db: dict):
        ...
        # 调用函数
        with patch("engram_step1.config.get_config", return_value=mock_config):
            with patch("engram_step1.db.get_config", return_value=mock_config):
                from engram_step1.db import create_item
                from engram_step1.errors import make_success_result
    ...
    def test_add_event_success_shape(self, migrated_db: dict):
        ...
        with patch("engram_step1.config.get_config", return_value=mock_config):
            with patch("engram_step1.db.get_config", return_value=mock_config):
                from engram_step1.db import create_item, add_event
                from engram_step1.errors import make_success_result
    ...
    def test_attach_success_shape(self, migrated_db: dict):
        ...
        with patch("engram_step1.config.get_config", return_value=mock_config):
            with patch("engram_step1.db.get_config", return_value=mock_config):
                from engram_step1.db import create_item, attach
                from engram_step1.errors import make_success_result
    ...
    def test_set_kv_success_shape(self, migrated_db: dict):
        ...
        with patch("engram_step1.config.get_config", return_value=mock_config):
            with patch("engram_step1.db.get_config", return_value=mock_config):
                from engram_step1.db import set_kv
                from engram_step1.errors import make_success_result
...
class TestLogbookCliErrorShape:
    ...
    def test_materialize_error_shape(self):
        ...
        from engram_step1.errors import MaterializeError, ChecksumMismatchError
...
class TestScmCliContractShape:
    """step1_cli.py SCM 命令的 JSON shape 测试（Mock 模式）"""
    ...
    def test_ensure_repo_success_shape_mock(self):
        ...
        from step1_cli import make_ok_result
    ...
    def test_error_response_shape_mock(self):
        ...
        from step1_cli import make_err_result
...
class TestGetBulkThresholds:
    ...
    def test_only_new_keys(self):
        """测试仅配置新键名时正确读取"""
        from unittest.mock import MagicMock
        from engram_step1.config import get_bulk_thresholds
...
class TestGetScmConfig:
    ...
    def test_only_new_keys(self):
        """测试仅配置新键名时正确读取"""
        from unittest.mock import MagicMock
        from engram_step1.config import get_scm_config
...
class TestRequireScmConfig:
    ...
    def test_missing_config_raises_error_with_new_key_hint(self):
        ...
        from unittest.mock import MagicMock
        from engram_step1.config import require_scm_config
        from engram_step1.errors import ConfigError
...
class TestGovernanceGetContractShape:
    ...
    def test_governance_get_exists_shape(self, migrated_db: dict):
        ...
        with patch("engram_step1.governance.get_connection") as mock_get_conn:
            # 创建模拟的数据库连接
            mock_conn = MagicMock()
            ...
            from engram_step1.governance import get_settings
            from engram_step1.errors import make_success_result
...
class TestExitCodeContract:
    ...
    def test_exit_code_values(self):
        """验证 ExitCode 常量值"""
        from engram_step1.errors import ExitCode
...
class TestOkFieldConsistency:
    ...
    def test_logbook_commands_use_ok(self):
        ...
        from engram_step1.errors import make_success_result, make_error_result
    ...
    def test_step1_cli_commands_use_ok(self):
        """验证 step1_cli 命令使用 ok 字段"""
        import sys
        from pathlib import Path
        ...
        from step1_cli import make_ok_result, make_err_result

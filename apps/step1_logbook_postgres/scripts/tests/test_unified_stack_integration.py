...
完整的 PostgreSQL + MinIO + Step1 集成测试，覆盖：
...
class TestArtifactWrite:
    ...
    def test_large_object_multipart_write(
        ...
        from engram_step1.artifact_store import MULTIPART_THRESHOLD
...
@integration_test
class TestStep1RolePermissions:
    """
    Step1 角色权限验证测试
    
    测试场景：
    1. step1_svc (继承 engram_app_readwrite) 在 Step1 schema (logbook, scm, analysis 等):
       - CREATE TABLE 应失败（无 DDL 权限）
       - SELECT/INSERT/UPDATE/DELETE 应成功（有 DML 权限）
    
    2. step1_migrator (继承 engram_migrator):
       - 应有 DDL 权限，可创建/修改表
    ...
    TEST_PASSWORD = "test_password_12345"
    STEP1_SCHEMAS = ["logbook", "scm", "analysis", "governance"]
    
    @pytest.fixture(scope="class")
    def setup_step1_test_roles(self, migrated_database):
        """
        创建/更新 Step1 测试用的登录角色
        
        创建:
        - step1_svc: 继承 engram_app_readwrite
        - step1_migrator: 继承 engram_migrator
        """
        import psycopg
        ...
        with conn.cursor() as cur:
            ...
            cur.execute("""
                DO $$
                BEGIN
                    IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'step1_svc') THEN
                        CREATE ROLE step1_svc LOGIN PASSWORD %s;
                        RAISE NOTICE 'Created test role: step1_svc';
                    ELSE
                        ALTER ROLE step1_svc WITH LOGIN PASSWORD %s;
                        RAISE NOTICE 'Updated test role: step1_svc';
                    END IF;
                    
                    IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'engram_app_readwrite') THEN
                        GRANT engram_app_readwrite TO step1_svc;
                    END IF;
                END $$;
            """, (self.TEST_PASSWORD, self.TEST_PASSWORD))
            
            # 创建 step1_migrator 登录角色
            cur.execute("""
                DO $$
                BEGIN
                    IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'step1_migrator') THEN
                        CREATE ROLE step1_migrator LOGIN PASSWORD %s;
                        RAISE NOTICE 'Created test role: step1_migrator';
                    ELSE
                        ALTER ROLE step1_migrator WITH LOGIN PASSWORD %s;
                        RAISE NOTICE 'Updated test role: step1_migrator';
                    END IF;
                    
                    IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'engram_migrator') THEN
                        GRANT engram_migrator TO step1_migrator;
                    END IF;
                END $$;
            """, (self.TEST_PASSWORD, self.TEST_PASSWORD))
        ...
        return {
            ...
            "dbname": parsed.path.lstrip("/") or "engram",
            "svc_user": "step1_svc",
            "migrator_user": "step1_migrator",
            "password": self.TEST_PASSWORD,
        }
    
    @pytest.fixture(scope="class")
    def svc_connection(self, setup_step1_test_roles):
        """使用 step1_svc 角色建立的数据库连接"""
        import psycopg
        
        config = setup_step1_test_roles
        ...
        try:
            ...
            pytest.skip(f"无法使用 step1_svc 连接: {e}")
    
    @pytest.fixture(scope="class")
    def migrator_connection(self, setup_step1_test_roles):
        """使用 step1_migrator 角色建立的数据库连接"""
        import psycopg
        ...
        try:
            ...
            pytest.skip(f"无法使用 step1_migrator 连接: {e}")
    
    @pytest.fixture
    def test_table_name(self):
        """生成唯一的测试表名"""
        unique_id = secrets.token_hex(4)
        return f"_test_step1_perm_{unique_id}"
    
    def test_svc_cannot_create_table_in_step1_schemas(
        self, svc_connection, test_table_name
    ):
        """
        验证 step1_svc 无法在 Step1 schema 内执行 CREATE TABLE
        
        预期: 抛出权限错误 (42501 - insufficient_privilege)
        """
        import psycopg
        
        for schema in self.STEP1_SCHEMAS:
            with svc_connection.cursor() as cur:
                with pytest.raises(psycopg.errors.InsufficientPrivilege):
                    cur.execute(f"""
    ...
    def test_svc_can_crud_on_existing_tables(
        ...
        """
        验证 step1_svc 可以对 migrator 创建的表执行 SELECT/INSERT/UPDATE/DELETE
        """
        import psycopg
    ...
    def test_migrator_can_create_tables_in_all_step1_schemas(
        self, migrator_connection, test_table_name
    ):
        """
        验证 step1_migrator 可以在所有 Step1 schema 内执行 CREATE TABLE
        """
        for schema in self.STEP1_SCHEMAS:
            with migrator_connection.cursor() as cur:
                # CREATE TABLE 应成功
    ...
    def test_svc_cannot_alter_or_drop_table(
        ...
        """
        验证 step1_svc 无法执行 ALTER TABLE 和 DROP TABLE
        """
        import psycopg
...
class TestSetRolePermissions:
    ...
    def test_set_role_step1_migrator_to_engram_migrator(
        self, setup_set_role_test, migrated_database
    ):
        """
        验证 step1_migrator 通过 SET ROLE 到 engram_migrator 时权限正确
        """
        import psycopg
        ...
        test_table = f"_test_step1_set_role_{secrets.token_hex(4)}"
        ...
        try:
            with conn.cursor() as cur:
                ...
                assert session_user == "step1_migrator"
    ...
    def test_step1_svc_dml_with_set_role(
        self, setup_set_role_test, migrated_database
    ):
        """
        验证 step1_svc 通过 SET ROLE 到 engram_app_readwrite 时 DML 权限正确
        """
        import psycopg
...
class TestCrossSchemaPermissions:
    ...
    验证 migrator 创建表后 svc 能正确 CRUD，同时覆盖 OpenMemory 与 Step1 schema。
    """
    ...
    def test_migrator_creates_svc_can_crud_step1_schema(
        self, setup_cross_schema_test
    ):
        """
        验证 Step1: migrator 创建表 -> svc 能 SELECT/INSERT/UPDATE/DELETE
        """
        import psycopg
        
        config = setup_cross_schema_test
        test_table = f"_test_cross_step1_{secrets.token_hex(4)}"
        schema = "logbook"
        
        # 使用 step1_migrator 连接创建表
        try:
    ...
    def test_both_schemas_in_single_transaction(
        ...
        """
        验证可以在同一事务中操作 Step1 和 OpenMemory schema（使用有权限的连接）
        """
        import psycopg
        ...
        test_table_step1 = f"_test_both_step1_{secrets.token_hex(4)}"
        ...
        try:
            with superuser_conn.cursor() as cur:
                # 创建两个 schema 的表
                cur.execute(f"""
                    CREATE TABLE logbook.{test_table_step1} (
                        id SERIAL PRIMARY KEY,
                        ref_id TEXT
                    )
                """)
                ...
                # 授予 step1_svc 权限
                cur.execute(f"GRANT SELECT, INSERT ON logbook.{test_table_step1} TO engram_app_readwrite")
                cur.execute(f"GRANT USAGE ON SEQUENCE logbook.{test_table_step1}_id_seq TO engram_app_readwrite")
                cur.execute(f"GRANT SELECT, INSERT ON {om_schema}.{test_table_om} TO engram_app_readwrite")
            
            # 使用 step1_svc 在两个 schema 中操作
            try:
                svc_conn = psycopg.connect(
                    host=config["host"],
                    port=config["port"],
            ...
            try:
                with svc_conn.cursor() as cur:
                    # 在 Step1 schema 插入
                    cur.execute(f"""
                        INSERT INTO logbook.{test_table_step1} (ref_id) 
                        VALUES ('ref_123')
                        RETURNING id
                    """)
                    step1_id = cur.fetchone()[0]
                    
                    # 在 OpenMemory schema 插入，引用 Step1 的 id
                    cur.execute(f"""
                        INSERT INTO {om_schema}.{test_table_om} (id, step1_id) 
                        VALUES ('mem_linked', %s)
                    """, (step1_id,))
                    ...
                    # 验证两边都有数据
                    cur.execute(f"SELECT COUNT(*) FROM logbook.{test_table_step1}")
            ...
            with superuser_conn.cursor() as cur:
                ...
                cur.execute(f"DROP TABLE IF EXISTS logbook.{test_table_step1}")

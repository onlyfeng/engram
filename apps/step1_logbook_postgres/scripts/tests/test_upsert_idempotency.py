...
class TestUpsertGitCommitIdempotency:
    ...
    def test_upsert_git_commit_idempotent(self, db_conn: psycopg.Connection):
        ...
        with db_conn.cursor() as cur:
            cur.execute(
                "SELECT message FROM git_commits WHERE git_commit_id = %s",
                (commit_id_1,)
            )
            row = cur.fetchone()
    ...
    def test_upsert_git_commit_source_id_persisted(self, db_conn: psycopg.Connection):
        ...
        with db_conn.cursor() as cur:
            cur.execute(
                "SELECT source_id FROM git_commits WHERE git_commit_id = %s",
                (commit_id,)
            )
            row = cur.fetchone()
            assert row[0] == expected_source_id, f"source_id 应为 {expected_source_id}"
            assert row[0] == f"git:{repo_id}:{commit_sha}", "source_id 格式应为 git:<repo_id>:<commit_sha>"
...
class TestUpsertSvnRevisionIdempotency:
    ...
    def test_svn_revision_overlap_sync_idempotent(self, db_conn: psycopg.Connection):
        ...
        with db_conn.cursor() as cur:
            cur.execute(
                "SELECT rev_num FROM svn_revisions WHERE repo_id = %s ORDER BY rev_num",
                (repo_id,)
            )
    ...
    def test_upsert_svn_revision_source_id_persisted(self, db_conn: psycopg.Connection):
        ...
        with db_conn.cursor() as cur:
            cur.execute(
                "SELECT source_id FROM svn_revisions WHERE svn_rev_id = %s",
                (rev_id,)
            )
            row = cur.fetchone()
            assert row[0] == expected_source_id, f"source_id 应为 {expected_source_id}"
            assert row[0] == f"svn:{repo_id}:{rev_num}", "source_id 格式应为 svn:<repo_id>:<rev_num>"
...
class TestUpsertMrIdempotency:
    ...
    def test_upsert_mr_idempotent(self, db_conn: psycopg.Connection):
        ...
        with db_conn.cursor() as cur:
            cur.execute(
                "SELECT status FROM mrs WHERE mr_id = %s",
                (mr_id,)
            )
            row = cur.fetchone()
            assert row[0] == "merged"
        ...
        with db_conn.cursor() as cur:
            cur.execute(
                "SELECT COUNT(*) FROM mrs WHERE mr_id = %s",
                (mr_id,)
            )
    ...
    def test_upsert_mr_source_id_persisted(self, db_conn: psycopg.Connection):
        ...
        with db_conn.cursor() as cur:
            cur.execute(
                "SELECT source_id FROM mrs WHERE mr_id = %s",
                (mr_id,)
            )
            row = cur.fetchone()
            assert row[0] == expected_source_id, f"source_id 应为 {expected_source_id}"
...
class TestUpsertPatchBlobIdempotency:
    ...
    def test_upsert_patch_blob_do_nothing(self, db_conn: psycopg.Connection):
        ...
        blob_id_1 = upsert_patch_blob(
            db_conn, source_type, source_id, sha256,
            uri=uri,
            size_bytes=1024,
        )
        db_conn.commit()
        ...
        with db_conn.cursor() as cur:
            cur.execute(
                "SELECT size_bytes FROM patch_blobs WHERE blob_id = %s",
                (blob_id_1,)
            )
            row = cur.fetchone()
...
class TestMrIdConsistencyAcrossScripts:
    ...
    def test_review_event_uses_consistent_mr_id(self, db_conn: psycopg.Connection):
        ...
        with db_conn.cursor() as cur:
            cur.execute(
                "SELECT mr_id FROM review_events WHERE id = %s",
                (event_id,)
            )
            row = cur.fetchone()
...
class TestStrictModeAndDiffMode:
    ...
    def test_strict_mode_no_cursor_advance_on_total_failure(self):
        ...
        # 非 strict 模式：仍然推进到 B
        target_commit_non_strict = all_commits[-1]
        assert target_commit_non_strict.sha == "bbb222", "非 strict 模式仍应推进到批次最后一个 commit"
...
class TestStrictModeIntegration:
    ...
    def test_sync_config_strict_default(self):
        ...
        from engram_step1.scm_auth import StaticTokenProvider

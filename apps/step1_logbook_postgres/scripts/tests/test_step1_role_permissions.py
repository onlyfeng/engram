...
验证 step1_migrator / step1_svc 登录角色的权限边界：
- step1_migrator: 继承 engram_migrator，拥有 DDL 权限（CREATE TABLE 等）
- step1_svc: 继承 engram_app_readwrite，仅 DML 权限（SELECT/INSERT/UPDATE/DELETE）

环境变量：
- STEP1_MIGRATOR_PASSWORD: step1_migrator 登录密码
- STEP1_SVC_PASSWORD: step1_svc 登录密码
- TEST_PG_DSN: 测试数据库 DSN（用于获取连接信息）

如果未设置密码环境变量，测试将使用 admin 连接临时创建服务账号。

运行方式：
- 集成测试: make test-step1-integration
- 单独运行: pytest tests/test_step1_role_permissions.py -v
...
ENV_SKIP_ROLE_PERMISSION_TESTS = "SKIP_STEP1_ROLE_PERMISSION_TESTS"
...
def role_test_setup(test_db_info: dict):
    ...
    2. 创建或更新 step1_migrator / step1_svc 登录角色
    3. 授予角色 membership
    4. 返回连接信息供测试使用
    ...
    migrator_password = get_password(ENV_STEP1_MIGRATOR_PASSWORD)
    svc_password = get_password(ENV_STEP1_SVC_PASSWORD)
    ...
    try:
        with admin_conn.cursor() as cur:
            ...
            # 2. 创建或更新 step1_migrator 登录角色
            cur.execute("SELECT 1 FROM pg_roles WHERE rolname = 'step1_migrator'")
            if cur.fetchone():
                cur.execute(f"ALTER ROLE step1_migrator WITH LOGIN PASSWORD %s", (migrator_password,))
            else:
                cur.execute(f"CREATE ROLE step1_migrator LOGIN PASSWORD %s", (migrator_password,))
            
            # 3. 创建或更新 step1_svc 登录角色
            cur.execute("SELECT 1 FROM pg_roles WHERE rolname = 'step1_svc'")
            if cur.fetchone():
                cur.execute(f"ALTER ROLE step1_svc WITH LOGIN PASSWORD %s", (svc_password,))
            else:
                cur.execute(f"CREATE ROLE step1_svc LOGIN PASSWORD %s", (svc_password,))
            
            # 4. 授予角色 membership
            cur.execute("GRANT engram_migrator TO step1_migrator")
            cur.execute("GRANT engram_app_readwrite TO step1_svc")
        ...
        yield {
            "admin_dsn": admin_dsn,
            "base_dsn": base_dsn,
            "migrator_dsn": build_user_dsn(base_dsn, "step1_migrator", migrator_password),
            "svc_dsn": build_user_dsn(base_dsn, "step1_svc", svc_password),
...
class TestStep1MigratorPermissions:
    """测试 step1_migrator 角色权限（DDL 权限）"""
    
    def test_migrator_can_connect(self, role_test_setup: dict):
        """测试 step1_migrator 可以登录数据库"""
        ...
        try:
            ...
            with conn.cursor() as cur:
                ...
                assert current_user == "step1_migrator", f"当前用户应为 step1_migrator，实际为 {current_user}"
            conn.close()
        except psycopg.OperationalError as e:
            pytest.fail(f"step1_migrator 无法登录: {e}")
    
    def test_migrator_can_create_table_in_logbook(self, role_test_setup: dict):
        """测试 step1_migrator 可以在 logbook schema 创建表"""
        migrator_dsn = role_test_setup["migrator_dsn"]
    ...
    def test_migrator_cannot_create_in_public(self, role_test_setup: dict):
        """测试 step1_migrator 不能在 public schema 创建表（strict 策略）"""
        migrator_dsn = role_test_setup["migrator_dsn"]
...
class TestStep1SvcPermissions:
    """测试 step1_svc 角色权限（仅 DML 权限）"""
    
    def test_svc_can_connect(self, role_test_setup: dict):
        """测试 step1_svc 可以登录数据库"""
        ...
        try:
            ...
            with conn.cursor() as cur:
                cur.execute("SELECT current_user")
                current_user = cur.fetchone()[0]
                assert current_user == "step1_svc", f"当前用户应为 step1_svc，实际为 {current_user}"
            conn.close()
        except psycopg.OperationalError as e:
            pytest.fail(f"step1_svc 无法登录: {e}")
    
    def test_svc_can_select_from_logbook_items(self, role_test_setup: dict):
        """测试 step1_svc 可以从 logbook.items 表查询"""
        svc_dsn = role_test_setup["svc_dsn"]
    ...
    def test_svc_can_insert_into_logbook_items(self, role_test_setup: dict):
        """测试 step1_svc 可以向 logbook.items 表插入数据"""
        svc_dsn = role_test_setup["svc_dsn"]
    ...
    def test_svc_can_update_logbook_items(self, role_test_setup: dict):
        """测试 step1_svc 可以更新 logbook.items 表"""
        svc_dsn = role_test_setup["svc_dsn"]
    ...
    def test_svc_can_delete_from_logbook_items(self, role_test_setup: dict):
        """测试 step1_svc 可以从 logbook.items 表删除数据"""
        svc_dsn = role_test_setup["svc_dsn"]
        ...
        try:
            with conn.cursor() as cur:
                ...
                try:
                    ...
                    """)
                    # 如果成功，这是个 bug
                    pytest.fail("step1_svc 不应能创建表（仅有 DML 权限）")
                except psycopg.errors.InsufficientPrivilege:
                    # 预期的权限不足错误
                    pass
        finally:
    ...
    def test_svc_cannot_drop_table(self, role_test_setup: dict):
        ...
        with admin_conn.cursor() as cur:
            ...
            cur.execute(f'GRANT SELECT ON {schema}."{temp_table}" TO step1_svc')
        admin_conn.close()
            ...
            try:
                with svc_conn.cursor() as cur:
                    ...
                    try:
                        ...
                        """)
                        pytest.fail("step1_svc 不应能修改表结构")
                    except psycopg.errors.InsufficientPrivilege:
                        # 预期的权限不足错误
                        pass
            finally:
...
class TestCrossSchemaPermissions:
    ...
    def test_svc_can_access_multiple_schemas(self, role_test_setup: dict):
        """测试 step1_svc 可以访问多个 Engram schema"""
        svc_dsn = role_test_setup["svc_dsn"]
        ...
        try:
            with conn.cursor() as cur:
                for schema in schemas_to_check:
                    ...
                    assert has_usage, f"step1_svc 应在 {schema} schema 有 USAGE 权限"
        finally:
            conn.close()
...
class TestDefaultPrivilegesWork:
    ...
    def test_new_table_by_migrator_accessible_by_svc(self, role_test_setup: dict):
        """测试 engram_migrator 创建的新表可被 step1_svc 访问"""
        migrator_dsn = role_test_setup["migrator_dsn"]
        svc_dsn = role_test_setup["svc_dsn"]
        admin_dsn = role_test_setup["admin_dsn"]

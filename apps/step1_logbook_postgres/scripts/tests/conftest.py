...
角色权限测试支持:
- 通过环境变量注入服务账号密码: STEP1_MIGRATOR_PASSWORD, STEP1_SVC_PASSWORD
...
from engram_step1.schema_context import SchemaContext, set_schema_context, reset_schema_context
...
def verify_search_path(conn, expected_schemas: dict) -> None:
    ...
    # 验证: public 必须是最后一个
    assert actual_schemas[-1] == "public", (
        f"search_path 中 public 应在最后，实际: {actual_path}"
    )
...
def roles_applied_db(test_db_info: dict) -> Generator[dict, None, None]:
    ...
    - 创建/更新 step1_migrator, step1_svc 登录角色
    - 返回包含角色连接信息的字典
    ...
    # 获取服务账号密码
    migrator_password = get_service_account_password(ENV_STEP1_MIGRATOR_PASSWORD)
    svc_password = get_service_account_password(ENV_STEP1_SVC_PASSWORD)
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            if cur.fetchone():
                cur.execute("ALTER ROLE step1_migrator WITH LOGIN PASSWORD %s", (migrator_password,))
            else:
                cur.execute("CREATE ROLE step1_migrator LOGIN PASSWORD %s", (migrator_password,))
            cur.execute("GRANT engram_migrator TO step1_migrator")
            
            # step1_svc
            cur.execute("SELECT 1 FROM pg_roles WHERE rolname = 'step1_svc'")
            if cur.fetchone():
                cur.execute("ALTER ROLE step1_svc WITH LOGIN PASSWORD %s", (svc_password,))
            else:
                cur.execute("CREATE ROLE step1_svc LOGIN PASSWORD %s", (svc_password,))
    ...
    yield {
        "dsn": dsn,
        "admin_dsn": admin_dsn,
        "migrator_dsn": build_user_dsn(dsn, "step1_migrator", migrator_password),
        "svc_dsn": build_user_dsn(dsn, "step1_svc", svc_password),
        "db_name": test_db_info["db_name"],
        "roles_applied": True,
    }

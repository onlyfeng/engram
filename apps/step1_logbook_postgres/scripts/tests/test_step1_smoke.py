...
class TestGetConnectionSearchPathUnit:
    ...
    def test_search_path_list_format(self):
        ...
        with patch('engram_step1.db.psycopg.connect', return_value=mock_conn):
            with patch('engram_step1.db.get_config', return_value=config):
    ...
    def test_search_path_uses_default_when_none_configured(self):
        ...
        from unittest.mock import MagicMock, patch
        from engram_step1.db import DEFAULT_SEARCH_PATH
        ...
        with patch('engram_step1.db.psycopg.connect', return_value=mock_conn):
            with patch('engram_step1.db.get_config', return_value=config):
                ...
                with patch('engram_step1.db.get_schema_context', side_effect=Exception("no context")):
                    from engram_step1.db import get_connection
                    conn = get_connection(config=config)
...
class TestGetConnectionSearchPath:
    ...
    def test_search_path_uses_default_when_empty(self, migrated_db: dict):
        ...
        from unittest.mock import MagicMock, patch
        from engram_step1.db import get_connection, DEFAULT_SEARCH_PATH
        from .conftest import get_test_dsn
...
class TestMigrateSelfCheckRegression:
    ...
    def test_required_trigger_exists_after_migration(self, migrated_db: dict):
        ...
        空库部署时可能缺失。现已添加到 01_step1_schema.sql。
        """
        scm_schema = migrated_db["schemas"]["scm"]
        ...
        try:
            with conn.cursor() as cur:
                ...
                assert exists, (
                    ...
                    "请确认 01_step1_schema.sql 中已定义该触发器。"
                )
        finally:
            conn.close()
...
class TestIdempotentOnErrorCommits:
    ...
    def test_commits_partial_success_idempotent(self, db_conn, requests_mock_fixture):
        ...
        """
        from unittest.mock import MagicMock, patch
        from engram_step1.gitlab_client import GitLabClient, HttpConfig, StaticTokenProvider
        import db as scm_db
...
class TestGetBulkThresholds:
    ...
    def test_partial_new_keys_fallback_to_old(self):
        """测试部分配置新键、部分回退旧键"""
        from unittest.mock import MagicMock
        from engram_step1.config import get_bulk_thresholds, DEFAULT_DIFF_SIZE_THRESHOLD

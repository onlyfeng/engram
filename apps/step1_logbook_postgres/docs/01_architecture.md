...
- openmemory.* 或 ${PROJECT_KEY}_openmemory.*：OpenMemory 自己的表（由其迁移管理，不手改）
  > ⚠️ **注意**：在启用 Step1 的 public CREATE 限制时，OpenMemory 不能使用 public schema，必须使用专用 schema
...
4. Step2/Step3 消费 Step1（异步或同步），但 Step1 始终是最终真相源
...
- OpenMemory 表使用专用 `openmemory` schema（推荐）或 `${PROJECT_KEY}_openmemory`
  > ⚠️ **重要**：在启用 Step1 的 public CREATE 限制时，OpenMemory 不能使用 public schema
...
```
> ⚠️ **注意**：在启用 Step1 的 public CREATE 限制时，OpenMemory 不能使用 public schema，必须配置专用 schema
...
- ⚠️ 在启用 Step1 的 public CREATE 限制时，OpenMemory 不能使用 public schema
...
| 环境变量 | 说明 | 示例值 |
|----------|------|--------|
| `PROJECT_KEY` | Step1 表前缀标识 | `proj_a` |
...
> ⚠️ **重要说明**：在启用 Step1 的 public CREATE 限制时，OpenMemory 不能使用 public schema。
...
ArtifactStore 是 Step1 事实层的制品存储组件，用于存储 patch/diff、日志、报告等证据文件。支持三种后端类型，各自适用于不同的部署场景。

            ...
            "test-job": {
                ...
                "steps": [
                    ...
                    },
                    {
                        ...
                        "uses": "actions/upload-artifact@v4",
                        "with": {
                            "name": "acceptance-runs",
                            "path": ".artifacts/acceptance-runs/"
                        }
                    }
                ]
            },
...
class TestCheckArtifactPathCoverage:
    ...
    def test_path_prefix_matching(self):
        """测试路径前缀匹配"""
        upload_steps = [
            {"step_name": "Upload", "paths": [".artifacts/acceptance-runs/run1.json"]}
        ]
    ...
    def test_step_name_filter(self):
        """测试 step name 过滤器"""
        upload_steps = [
            {"step_name": "Upload verification results", "paths": [".artifacts/verify-results.json"]},
    ...
    def test_empty_required_paths(self):
        ...
        upload_steps = [
            {"step_name": "Upload", "paths": [".artifacts/something/"]}
        ]
        required_paths = []
...
class TestValidateArtifactArchive:
    ...
    def test_artifact_archive_with_step_filter(self, temp_workspace):
        """测试带 step name 过滤的 artifact archive"""
        import yaml
                    ...
                    "steps": [
                        {
                            "name": "Upload verification results",
                            "uses": "actions/upload-artifact@v4",
                            "with": {"path": ".artifacts/verify-results.json"}
                        },
                        {
                            "name": "Upload other stuff",
                            "uses": "actions/upload-artifact@v4",
                            "with": {"path": ".artifacts/other/"}
                        }
                    ]
                }
            }
        ...
        contract = {
            ...
            "ci": {
                ...
                "artifact_archive": {
                    ...
                    "artifact_step_names": ["verification"]
                }
            }
        }
...
"""
    ...
    def test_nightly_acceptance_artifacts(self, temp_workspace):
        """测试 nightly workflow 的 acceptance artifacts 验证"""
  ...
  nightly-full:
    ...
    steps:
      ...
      - name: Upload acceptance-unified-full results
        ...
        uses: actions/upload-artifact@v4
        with:
          name: nightly-acceptance-unified-full
          path: |
...
"""
        ...
        artifact_errors = [e for e in result.errors if e.error_type == "missing_artifact_path"]
        assert len(artifact_errors) == 0, f"Unexpected errors: {[e.key for e in artifact_errors]}"

...
gate_profile 映射（与 AGENTS.md 子代理分工对齐）:
    ...
    - ci-only: CI 代理（typecheck, validate-workflows-strict, check-workflow-contract-docs-sync, check-workflow-contract-version-policy）
...
# gate_profile 类型定义
GateProfile = Literal["full", "docs-only", "ci-only", "gateway-only", "sql-only"]

# 支持的 profile 列表
SUPPORTED_PROFILES: List[GateProfile] = ["full", "docs-only", "ci-only", "gateway-only", "sql-only"]
...
# expected_keyword 与 Makefile echo 输出对齐
FULL_GATE_COMMANDS: List[GateCommand] = [
    GateCommand("make lint", "代码风格检查（ruff check）", "All checks passed"),
    GateCommand("make format-check", "格式检查（ruff format --check）", "already formatted"),
    GateCommand("make typecheck", "mypy 类型检查", "Success"),
    GateCommand("make check-schemas", "JSON Schema 校验", "Schema 校验通过"),
    GateCommand("make check-env-consistency", "环境变量一致性检查", "环境变量一致性检查通过"),
    GateCommand(
        "make check-logbook-consistency", "Logbook 配置一致性检查", "Logbook 配置一致性检查通过"
    ),
    GateCommand(
        "make check-migration-sanity", "SQL 迁移文件检查", "SQL 迁移文件检查通过"
    ),
    GateCommand(
    ...
    GateCommand(
        ...
        "Gateway ErrorReason 使用规范检查通过",
    ),
    GateCommand(
        "make check-gateway-public-api-surface",
        "Gateway Public API 导入表面检查",
        "Gateway Public API 导入表面检查通过",
    ),
    GateCommand(
        "make check-gateway-public-api-docs-sync",
        "Gateway Public API 文档同步检查",
        "Gateway Public API 文档同步检查通过",
    ),
    GateCommand(
        "make check-gateway-di-boundaries", "Gateway DI 边界检查", "Gateway DI 边界检查通过"
    ),
    GateCommand(
        "make check-gateway-import-surface",
        "Gateway 懒加载策略检查",
        "Gateway Import Surface 检查通过",
    ),
    GateCommand(
        "make check-gateway-correlation-id-single-source",
        "Gateway correlation_id 单一来源检查",
        "Gateway correlation_id 单一来源检查通过",
    ),
    GateCommand("make check-iteration-docs", "迭代文档规范检查", "迭代文档规范检查通过"),
    GateCommand(
    ...
    GateCommand(
        "make check-mcp-error-contract", "MCP 错误码合约检查", "MCP JSON-RPC 错误码合约检查通过"
    ),
    GateCommand(
        "make check-mcp-error-docs-sync",
        "MCP 错误码文档同步检查",
        "MCP JSON-RPC 错误码文档同步检查通过",
    ),
]
...
SQL_GATE_COMMANDS: List[GateCommand] = [
    ...
    GateCommand("make verify-permissions", "数据库权限验证", "权限验证完成"),
]
...
def render_command_table(commands: List[GateCommand]) -> str:
    ...
    for i, cmd in enumerate(commands, 1):
        lines.append(f"| {i} | `{cmd.command}` | {cmd.description} | `{cmd.expected_keyword}` |")
...
def render_expected_keywords(commands: List[GateCommand]) -> str:
    """渲染预期关键字说明。
    ...
    Returns:
        预期关键字说明字符串
    """
    lines = ["**通过标准**：每个命令的输出应包含对应的预期关键字。", ""]

    for cmd in commands:
        lines.append(f"- `{cmd.command}` → 输出包含 `{cmd.expected_keyword}`")
...
def render_min_gate_block(
    ...
    Args:
        ...
        profile: 门禁 profile（full/docs-only/ci-only/gateway-only/sql-only）
    ...
    sections = [
        ...
        "### 预期关键字",
        "",
        render_expected_keywords(commands),
        "",
    ]

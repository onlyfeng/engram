    ...
    def test_migrator_can_connect(self, role_test_setup: dict):
        ...
        try:
            ...
            with conn.cursor() as cur:
                ...
                assert current_user == "logbook_migrator", (
                    f"当前用户应为 logbook_migrator，实际为 {current_user}"
                )
            conn.close()
    ...
    def test_svc_can_connect(self, role_test_setup: dict):
        ...
        try:
            ...
            with conn.cursor() as cur:
                cur.execute("SELECT current_user")
                current_user = cur.fetchone()[0]
                assert current_user == "logbook_svc", (
                    f"当前用户应为 logbook_svc，实际为 {current_user}"
                )
            conn.close()
        except psycopg.OperationalError as e:
            ...
            with conn.cursor() as cur:
                for schema in schemas_to_check:
                    ...
                    assert not has_create, (
                        f"engram_app_readwrite 不应在 {schema} schema 有 CREATE 权限"
                    )
        finally:
            conn.close()

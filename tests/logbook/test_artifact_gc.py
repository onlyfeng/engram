...
class TestAuditEventPayload:
    ...
    def test_audit_event_optional_fields(self):
        ...
        event = AuditEvent(
            tool="artifact_gc",
            operation="gc_move_to_trash",
            success=True,
            backend="object",
            uri="scm/proj/1/test.diff",
            bucket="engram-bucket",
            object_key="prefix/scm/proj/1/test.diff",
            trash_prefix=".trash/",
        ...
        assert event.object_key == "prefix/scm/proj/1/test.diff"
        assert event.trash_prefix == ".trash/"
        assert event.using_ops_credentials is True
        assert event.error is None
        assert event.details["size_bytes"] == 1024
        assert event.details["age_days"] == 30.5

    def test_audit_event_to_dict(self):
        """验证 AuditEvent.to_dict() 正确排除 None 值"""
        event = AuditEvent(
        ...
        assert "success" in d
        assert "error" in d
        # None 值应被排除（backend, uri 等未设置）
        assert d.get("backend") is None or "backend" not in d
        assert d.get("uri") is None or "uri" not in d

    def test_gc_delete_audit_event_has_required_fields(self):
        ...
        with patch("engram_logbook.artifact_ops_audit.write_audit_event") as mock_write:
            ...
            assert event.using_ops_credentials is True
            assert event.success is True
            assert event.details.get("require_ops") is True
            assert event.details.get("size_bytes") == 2048
            assert event.details.get("age_days") == 15.5

    def test_gc_delete_audit_event_hard_delete_operation(self):
    ...
    def test_gc_summary_audit_event_has_required_fields(self):
        ...
        with patch("engram_logbook.artifact_ops_audit.write_audit_event") as mock_write:
            ...
            assert event.details.get("scanned") == 1000
            assert event.details.get("protected") == 800
            assert event.details.get("candidates") == 200
            assert event.details.get("deleted") == 150
            assert event.details.get("trashed") == 45
            assert event.details.get("failed") == 5
            assert event.details.get("require_ops") is True

    def test_delete_audit_event_has_required_fields(self):
    ...
    def test_delete_audit_event_move_to_trash_operation(self):
...
class TestGCAuditIntegration:
    """测试 GC 与审计系统的集成"""

    def test_run_gc_writes_audit_events_on_delete(
        self,
        artifacts_root: Path,
    ):
        """GC 删除时应写入审计事件"""
        ...
        with (
            ...
            # 验证删除审计事件被调用
            assert mock_delete_audit.call_count == 1
            delete_call = mock_delete_audit.call_args
            assert delete_call.kwargs["uri"] == "scm/orphan.diff"
            assert delete_call.kwargs["backend"] == "local"
            assert delete_call.kwargs["success"] is True

            # 验证汇总审计事件被调用
            mock_summary_audit.assert_called_once()
            summary_call = mock_summary_audit.call_args
            assert summary_call.kwargs["gc_mode"] == "orphan"
            assert summary_call.kwargs["backend"] == "local"
            assert summary_call.kwargs["scanned"] == 1
            assert summary_call.kwargs["deleted"] == 1

    def test_run_gc_no_audit_in_dry_run(
        self,
        artifacts_root: Path,
    ):
        """GC dry-run 模式不应写入审计事件"""
        scm_dir = artifacts_root / "scm"
        scm_dir.mkdir()
        (scm_dir / "orphan.diff").write_bytes(b"orphan content")
        ...
        with (
            ...
            # dry-run 模式不应写入审计事件
            mock_delete_audit.assert_not_called()
            mock_summary_audit.assert_not_called()

    def test_run_gc_audit_disabled(
        self,
        artifacts_root: Path,
    ):
        """audit=False 时不应写入审计事件"""
        scm_dir = artifacts_root / "scm"
        scm_dir.mkdir()
        (scm_dir / "orphan.diff").write_bytes(b"orphan content")
    ...
    def test_run_tmp_gc_writes_audit_events(
        self,
        artifacts_root: Path,
    ):
        """TMP GC 删除时应写入审计事件"""
        ...
        with (
            ...
            delete_call = mock_delete_audit.call_args
            assert "tmp/old.tar.gz" in delete_call.kwargs["uri"]
            assert delete_call.kwargs["success"] is True

            # 验证汇总审计事件
            mock_summary_audit.assert_called_once()
            summary_call = mock_summary_audit.call_args
            assert summary_call.kwargs["gc_mode"] == "tmp"
            assert summary_call.kwargs["deleted"] == 1

    def test_gc_audit_records_require_ops_for_object_backend(
        self,
        artifacts_root: Path,
    ):
        """验证 object 后端的审计事件记录 require_ops 字段"""
        # 这个测试主要验证 require_ops 参数被正确传递
        # 由于我们不实际连接 object 后端，使用 local 后端模拟
        scm_dir = artifacts_root / "scm"
        scm_dir.mkdir()
        (scm_dir / "orphan.diff").write_bytes(b"orphan content")
        ...
        with (
            ...
            run_gc(
                ...
                delete=True,
                dsn="mock://",
                backend="local",  # local 后端不检查 require_ops
                artifacts_root=str(artifacts_root),
                verbose=False,
                require_ops=True,  # 设置 require_ops
                audit=True,
            )

            # 验证审计事件被调用（local 后端 require_ops 为 None）
            mock_delete_audit.assert_called_once()
            # local 后端的 require_ops 应为 None（因为不适用）
            delete_call = mock_delete_audit.call_args
            assert delete_call.kwargs.get("require_ops") is None

    def test_gc_audit_records_failed_deletes(
        self,
        artifacts_root: Path,
    ):
        """验证删除失败时审计事件记录错误信息"""
        ...
        try:
            with (
                ...
                mock_delete_audit.assert_called_once()
                delete_call = mock_delete_audit.call_args
                assert delete_call.kwargs["success"] is False
                assert delete_call.kwargs["error"] is not None

                # 验证汇总事件记录了失败数
                mock_summary_audit.assert_called_once()
                summary_call = mock_summary_audit.call_args
                assert summary_call.kwargs["failed"] == 1
        finally:
            # 恢复权限以便清理
            scm_dir.chmod(0o755)
...
class TestAuditEventWriteFunction:
    """测试审计事件写入函数"""

    def test_write_audit_event_without_dsn_returns_none(self):
    ...
    def test_audit_event_details_serialized_as_json(self):
        """验证 details 字段被正确序列化为 JSON"""
        event = AuditEvent(
            tool="artifact_gc",
...
# =============================================================================
# 测试: GC 治理开关（require_trash / require_ops 默认值与覆盖）
# =============================================================================


class TestGCGovernanceSwitches:
    """测试 GC 治理开关的默认值读取与显式覆盖行为"""

    def test_require_trash_default_false_allows_hard_delete(
        self,
        artifacts_root: Path,
        sample_artifacts: dict,
    ):
    ...
    def test_require_trash_env_true_blocks_hard_delete(
        self,
        artifacts_root: Path,
        sample_artifacts: dict,
    ):
    ...
    def test_require_trash_with_trash_prefix_always_succeeds(
        self,
        artifacts_root: Path,
        sample_artifacts: dict,
    ):
    ...
    def test_require_ops_env_default_object_backend(self, artifacts_root: Path):
        """
        require_ops 默认值从环境变量读取（object 后端）
        """
        # 创建测试文件
        scm_dir = artifacts_root / "scm"
        scm_dir.mkdir()
        ...
        with patch.dict(
            ...
            with pytest.raises(GCOpsCredentialsRequiredError):
                run_gc(
                    ...
                    delete=True,
                    backend="object",
                    require_ops=None,  # 使用默认值（来自环境变量 True）
                    verbose=False,
                )

    def test_require_ops_explicit_false_overrides_env(self, artifacts_root: Path):
        """
        显式 require_ops=False 覆盖环境变量默认值
        """
        # 创建测试文件
        scm_dir = artifacts_root / "scm"
        scm_dir.mkdir()
...
class TestGCGovernanceConfigIntegration:
    """测试 GC 治理开关配置集成"""

    def test_get_gc_governance_config_default_values(self):
        """
    ...
    def test_get_gc_governance_config_env_override(self):
        """
    ...
    def test_get_gc_governance_config_env_false_values(self):
        """
        测试环境变量设置为 false 的情况
        """
        from engram.logbook.config import get_gc_governance_config
    ...
    def test_get_gc_require_trash_default_function(self):
        """
    ...
    def test_get_gc_require_ops_default_function(self):
        """
...
class TestGCGovernanceTmpMode:
    """测试 tmp 清理模式的治理开关"""

    def test_tmp_gc_require_ops_from_env(self, artifacts_root: Path):
        """
        tmp 清理模式从环境变量读取 require_ops 默认值
        """
        # 创建 tmp 文件
        tmp_dir = artifacts_root / "tmp"
        tmp_dir.mkdir()
        ...
        with patch.dict(
            ...
            with pytest.raises(GCOpsCredentialsRequiredError):
                run_tmp_gc(
                    ...
                    delete=True,
                    backend="object",
                    require_ops=None,  # 使用默认值
                    verbose=False,
                )

    def test_tmp_gc_require_ops_explicit_false(self, artifacts_root: Path):
        """
        tmp 清理模式显式 require_ops=False 覆盖默认值
        """
        # 创建 tmp 文件
        tmp_dir = artifacts_root / "tmp"
        tmp_dir.mkdir()

...
class TestReviewEventsDedup:
    ...
    def test_reviewer_user_id_null_when_not_resolved(self, db_conn):
        ...
        with db_conn.cursor() as cur:
            cur.execute("SELECT reviewer_user_id FROM review_events WHERE id = %s", (event_id,))
            row = cur.fetchone()
...
class TestBackfillIdempotency:
    ...
    def test_backfill_twice_no_duplicate_events(self, db_conn):
        ...
        events_to_insert = [
            {
                "source_event_id": f"note:backfill-1-{ts}",
                "event_type": "comment",
                "payload": {"body": "LGTM"},
            },
            {
                "source_event_id": f"note:backfill-2-{ts}",
                "event_type": "comment",
                "payload": {"body": "Nice work"},
            },
            {
                "source_event_id": f"approval:{mr_id}:user1-{ts}",
                "event_type": "approve",
                "payload": {"approved": True},
            },
            {
                "source_event_id": f"state:backfill-3-{ts}",
                "event_type": "merge",
        ...
        # 第一次回填应该全部成功插入
        assert first_run_inserted == len(events_to_insert), (
            f"第一次回填应全部插入，实际插入 {first_run_inserted}"
        )
        ...
        assert second_run_skipped == len(events_to_insert), (
            f"第二次回填应全部跳过，实际跳过 {second_run_skipped}"
        )
        ...
        with db_conn.cursor() as cur:
            ...
            assert total_count == len(events_to_insert), (
                f"应该只有 {len(events_to_insert)} 条记录，实际有 {total_count}"
            )
...
class TestHttpErrorHandling:
    ...
    def test_403_triggers_token_invalidate_only_once(self, requests_mock):
        ...
        from engram.logbook.gitlab_client import GitLabAuthError, GitLabClient, HttpConfig
...
class TestCommitsIdempotentOnError:
    ...
    def test_commit_diff_failure_continues(self, db_conn, requests_mock):
        ...
        upsert_repo(db_conn, "git", url, "test_project")
        db_conn.commit()

...
class TestBudgetInvariants:
    ...
    def test_invariant_per_instance_concurrency_never_exceeded(self):
        ...
        for initial_instance_count in [0, 1, 2]:
            ...
            instance_candidates = sum(
                ...
                if next((s for s in states if s.repo_id == c.repo_id)).gitlab_instance == instance_key
            )
        ...
        budget_full = BudgetSnapshot(
            global_running=3, global_active=3,
    ...
    def test_invariant_per_tenant_concurrency_never_exceeded(self):
        ...
        # 额外测试：租户已满时返回空
        budget_full = BudgetSnapshot(
            global_running=2, global_active=2,
    ...
    def test_invariant_combined_limits_all_respected(self):
        ...
        states = [
            # 实例 A，租户 1
            RepoSyncState(repo_id=1, repo_type="git", gitlab_instance="instance-a", tenant_id="tenant-1", cursor_updated_at=now - 5000),
            RepoSyncState(repo_id=2, repo_type="git", gitlab_instance="instance-a", tenant_id="tenant-1", cursor_updated_at=now - 5000),
            RepoSyncState(repo_id=3, repo_type="git", gitlab_instance="instance-a", tenant_id="tenant-1", cursor_updated_at=now - 5000),
            # 实例 A，租户 2
            RepoSyncState(repo_id=4, repo_type="git", gitlab_instance="instance-a", tenant_id="tenant-2", cursor_updated_at=now - 5000),
            RepoSyncState(repo_id=5, repo_type="git", gitlab_instance="instance-a", tenant_id="tenant-2", cursor_updated_at=now - 5000),
            # 实例 B，租户 1
            RepoSyncState(repo_id=6, repo_type="git", gitlab_instance="instance-b", tenant_id="tenant-1", cursor_updated_at=now - 5000),
            RepoSyncState(repo_id=7, repo_type="git", gitlab_instance="instance-b", tenant_id="tenant-1", cursor_updated_at=now - 5000),
            # 实例 B，租户 2
            RepoSyncState(repo_id=8, repo_type="git", gitlab_instance="instance-b", tenant_id="tenant-2", cursor_updated_at=now - 5000),
            RepoSyncState(repo_id=9, repo_type="git", gitlab_instance="instance-b", tenant_id="tenant-2", cursor_updated_at=now - 5000),
            RepoSyncState(repo_id=10, repo_type="git", gitlab_instance="instance-b", tenant_id="tenant-2", cursor_updated_at=now - 5000),
        ]
        ...
        for c in candidates:
            ...
            if state.gitlab_instance:
                instance_counts[state.gitlab_instance] = instance_counts.get(state.gitlab_instance, 0) + 1
            if state.tenant_id:
    ...
    def test_operation_counts_basic(self):
        ...
        with patch.object(policy_module, 'compute_job_priority', counting_compute_priority):
            with patch.object(policy_module, 'should_schedule_repo_health', counting_should_schedule):
                candidates = select_jobs_to_enqueue(
                    states, job_types, config, now=now,
                    budget_snapshot=BudgetSnapshot.empty(),
                )

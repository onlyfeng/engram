...
class DataFactory:
    ...
    def create_repo(
        self, vcs_type: str = "git", remote_url: Optional[str] = None, **kwargs
    ) -> MockRepo:
        """
        创建测试仓库
    ...
    def create_expired_job(
        self, repo_id: int, minutes_expired: int = 10, lease_seconds: int = 300, **kwargs
    ) -> MockJob:
        """
        创建过期的 running 任务
    ...
    def create_expired_run(self, repo_id: int, minutes_running: int = 45, **kwargs) -> MockRun:
        """
        创建超时的 running run
        ...
        return self.create_run(repo_id=repo_id, status="running", started_at=started_at, **kwargs)
...
def data_factory_context(
    conn: psycopg.Connection, scm_schema: str = "scm"
...
class JobAssertions:
    ...
    def assert_locked_by(self, job_id: str, worker_id: str, msg: str = None):
        ...
        assert state["locked_by"] == worker_id, (
            msg
            or f"Expected job {job_id} to be locked by '{worker_id}', got '{state['locked_by']}'"
        )
    ...
    def assert_has_backoff(self, job_id: str, min_seconds: int = 0, msg: str = None):
        ...
        if not_before is None:
            ...
            assert False, msg or f"Expected job {job_id} to have backoff, but not_before is None"
...
class SensitiveDataAssertions:
    ...
    def assert_no_sensitive_data(cls, text: str, msg: str = None):
        ...
        for pattern in cls.SENSITIVE_PATTERNS:
            ...
            if match:
                assert False, (
                    msg
                    or f"Found sensitive data pattern '{pattern}' in text: {match.group()[:20]}..."
                )
    ...
    def assert_is_redacted(cls, text: str, msg: str = None):
        ...
        assert has_marker, msg or f"Expected text to contain redaction marker, got: {text[:100]}..."

    @classmethod
    def assert_token_redacted(cls, original_token: str, processed_text: str, msg: str = None):
        """
...
class MockHelpers:
    ...
    def make_success_result(
        run_id: str = "run-123", synced_count: int = 10, **extra
    ) -> Dict[str, Any]:
        """创建成功的执行结果"""
        result = {
            "success": True,
    ...
    def make_skipped_result(
        message: str = "Already running on another worker", **extra
    ) -> Dict[str, Any]:
        """创建 skipped 的执行结果"""
        result = {
            "success": True,
...
class BackoffAssertions:
    ...
    def assert_backoff_seconds(
        cls, actual: int, error_category: str, msg: str = None, tolerance: int = 5
    ):
        """断言退避时间符合预期"""
    ...
    def assert_fail_retry_called_with_backoff(
        cls, mock_fail_retry: MagicMock, error_category: str, msg: str = None
    ):
        ...
        assert actual_backoff == expected_backoff, (
            msg
            or f"Expected fail_retry backoff={expected_backoff}s for {error_category}, got {actual_backoff}s"
        )
...
class CircuitBreakerAssertions:
    ...
    def assert_state(controller, expected_state: str, msg: str = None):
        """断言熔断器状态"""
        actual = (
            controller.state.value if hasattr(controller.state, "value") else str(controller.state)
        )
    ...
    def assert_degraded(decision, msg: str = None):
        ...
        assert decision.is_backfill_only is True, (
            msg
            or f"Expected decision to be degraded (backfill_only), got is_backfill_only={decision.is_backfill_only}"
        )

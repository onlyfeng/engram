...
class TestReaperFixturesUnit:
    ...
    def test_permanent_error_classification_fixture(self):
        """验证永久性错误夹具全部分类正确"""
        from engram.logbook.scm_sync_errors import classify_last_error, PERMANENT_ERROR_CATEGORIES
    ...
    def test_transient_error_classification_fixture(self):
        """验证临时性错误夹具全部分类正确"""
        from engram.logbook.scm_sync_errors import classify_last_error, TRANSIENT_ERROR_CATEGORIES
...
class TestReaperIdempotency:
    ...
    def test_reap_expired_run_is_idempotent(self, migrated_db):
        ...
        try:
            ...
            try:
                # 第一次执行 reaper
                expired_runs = list_expired_running_runs(test_conn, max_duration_seconds=1800, limit=10)
                ...
                expired_runs_2 = list_expired_running_runs(test_conn, max_duration_seconds=1800, limit=10)
    ...
    def test_reap_expired_lock_is_idempotent(self, migrated_db):
        ...
        try:
            ...
            try:
                ...
                expired_lock_ids = [l["lock_id"] for l in expired_locks_2]
...
class TestWorkerReaperBackoffConsistency:
    ...
    def test_worker_exception_handling_uses_resolve_backoff(self):
        ...
        from engram.logbook.scm_sync_errors import (
            resolve_backoff, BackoffSource,
            TRANSIENT_ERROR_BACKOFF, ErrorCategory,
        )
        ...
        exception_errors = [
            ("Request timeout after 30 seconds", "timeout", TRANSIENT_ERROR_BACKOFF[ErrorCategory.TIMEOUT.value]),
            ("429 Too Many Requests", "rate_limit", TRANSIENT_ERROR_BACKOFF[ErrorCategory.RATE_LIMIT.value]),
            ("Connection reset by peer", "network", TRANSIENT_ERROR_BACKOFF[ErrorCategory.NETWORK.value]),
            ("502 Bad Gateway", "server_error", TRANSIENT_ERROR_BACKOFF[ErrorCategory.SERVER_ERROR.value]),
    ...
    def test_backoff_source_tracking(self):
        """验证 backoff 来源追踪正确"""
        from engram.logbook.scm_sync_errors import resolve_backoff, BackoffSource
    ...
    def test_worker_reaper_shared_constants(self):
        ...
        for category in TRANSIENT_ERROR_CATEGORIES:
            assert category in TRANSIENT_ERROR_BACKOFF or category == ErrorCategory.LEASE_LOST.value, \

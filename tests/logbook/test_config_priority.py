...
""")
        ...
        tmp_path / "home" / ".agentx" / "config.toml"
...
class TestConfigNotFoundError:
    ...
    def test_all_paths_missing_allows_empty_config(self, tmp_path: Path):
        ...
        with patch.dict(os.environ, env, clear=True):
            with patch(
                "engram_logbook.config.DEFAULT_CONFIG_PATHS", [nonexistent_cwd, nonexistent_home]
            ):
                config = Config()
                config.load()
...
class TestConfigErrorHintMessage:
    ...
    def test_config_not_found_error_has_hint(self, tmp_path: Path):
        ...
        error = exc_info.value
        # 错误信息应该包含路径
        assert str(nonexistent) in error.message or str(nonexistent.absolute()) in str(
            error.details
        )

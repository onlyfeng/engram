...
class TestMigrateConcurrent:
    ...
    def test_concurrent_migrate_same_prefix_serialized(self, test_db_info):
        ...
        p1 = multiprocessing.Process(
            target=run_migrate_in_process, args=(dsn, schema_prefix, result_queue)
        )
    ...
    def test_concurrent_migrate_different_prefix_parallel(self, test_db_info):
        ...
        p1 = multiprocessing.Process(
            target=run_migrate_in_process, args=(dsn, prefix1, result_queue)
        )
        p2 = multiprocessing.Process(
            target=run_migrate_in_process, args=(dsn, prefix2, result_queue)
        )
    ...
    def test_migrate_acquires_and_releases_lock(self, test_db_info):
        ...
        with psycopg.connect(dsn, autocommit=True) as conn:
            with conn.cursor() as cur:
                # 尝试以非阻塞方式获取锁，应该成功
                cur.execute(
                    "SELECT pg_try_advisory_lock(hashtext(%s))", ("engram_migrate:" + prefix,)
                )
                ...
                # 释放测试锁
                cur.execute(
                    "SELECT pg_advisory_unlock(hashtext(%s))", ("engram_migrate:" + prefix,)
                )
...
class TestDatabaseAutoCreateConcurrent:
    ...
    def test_concurrent_database_creation_idempotent(self, test_db_info):
        ...
        try:
            # 启动两个进程同时创建
            p1 = multiprocessing.Process(
                target=create_in_process, args=(target_dsn, admin_dsn, new_db_name, result_queue)
            )
            p2 = multiprocessing.Process(
                target=create_in_process, args=(target_dsn, admin_dsn, new_db_name, result_queue)
            )

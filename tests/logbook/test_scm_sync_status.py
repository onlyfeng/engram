...
class TestPrometheusOutputFormat:
    ...
    def test_prometheus_rate_limit_pause_until_metric(self, db_conn):
        ...
        match = re.search(
            r'scm_rate_limit_pause_until\{instance_key="[^"]+"\}\s+(\d+)', prom_output
        )
...
class TestNoSensitiveInfoLeakage:
    ...
    def test_top_lag_repos_url_redacted(self, db_conn):
        ...
        if found:
            ...
            url = repo.get("url", "")
            repo.get("project_key", "")

            # GitLab token 应被替换
            assert "glpat-secret123456789" not in (url or ""), (
                "GitLab token should be redacted in URL"
            )
            # 用户凭证应被替换
...
class TestPrometheusMetricLabelsStability:
    ...
    def test_jobs_by_status_labels_are_normalized(self, db_conn):
        ...
        assert (
            'instance_key="private.gitlab.com"' in prom_output
            or 'instance_key="_unknown_"' in prom_output
        )
...
class TestErrorBudgetRealAggregation:
    """测试 error_budget 从 scm.sync_runs 真实聚合数据"""

    def test_error_budget_aggregates_from_sync_runs(self, db_conn):
        """验证 error_budget 从 sync_runs 表真实聚合统计"""
        # 创建测试仓库
        repo_id = upsert_repo(
            db_conn,
            repo_type="git",
            url="https://example.com/error-budget-test.git",
            project_key="error_budget_test",
        )

        # 插入一些 sync_runs 记录，包含 counts 和 error_summary_json
        # 3 个完成的 run
        for i in range(3):
            run_id = str(uuid.uuid4())
            insert_sync_run_start(db_conn, run_id, repo_id, "gitlab_commits")
        ...
        for i in range(2):
            ...
            insert_sync_run_finish(
                ...
                error_summary_json={"error_type": "API_ERROR", "message": "test error"},
            )
        ...
        # 验证聚合数据（至少包含我们插入的 5 条记录）
        assert error_budget["samples"] >= 5
        assert error_budget["completed_runs"] >= 3

        # 验证 failure 统计
        assert "failure" in error_budget
        assert error_budget["failure"]["count"] >= 2
        # failure_rate = 2 / 5 = 0.4
        assert error_budget["failure"]["rate"] > 0

        # 验证 rate_limit_429 统计
        # total_429_hits = 3*2 + 2*5 = 16
        # total_requests = 3*20 + 2*10 = 80
        assert "rate_limit_429" in error_budget
        assert error_budget["rate_limit_429"]["count"] >= 16

        # 验证 timeout 统计
        # total_timeout_count = 3*1 + 2*3 = 9
        assert "timeout" in error_budget
        assert error_budget["timeout"]["count"] >= 9

    def test_error_budget_empty_when_no_runs(self, db_conn):
        """验证没有 sync_runs 时 error_budget 返回零值"""
        import os
        import sys
        ...
        # 空数据时应返回零值
        assert error_budget["failure"]["rate"] == 0.0
        assert error_budget["rate_limit_429"]["rate"] == 0.0
        assert error_budget["timeout"]["rate"] == 0.0

    def test_prometheus_error_budget_reflects_real_data(self, db_conn):
        """验证 Prometheus 输出反映真实的 error_budget 数据"""
        # 创建测试仓库和一些 sync_runs
        repo_id = upsert_repo(
            db_conn,
            repo_type="git",
            url="https://example.com/prom-error-budget-test.git",
            project_key="prom_error_budget",
        )
        ...
        insert_sync_run_finish(
            ...
            run_id,
            status="failed",
            counts={"total_requests": 100, "total_429_hits": 25, "total_timeout_count": 10},
        )
        ...
        # 验证 Prometheus 输出包含真实的 error_budget 数据
        assert "scm_error_budget_samples" in prom_output
        assert "scm_error_budget_failure_count" in prom_output
        assert "scm_error_budget_429_count" in prom_output
        assert "scm_error_budget_timeout_count" in prom_output
        ...
        # 至少有 1 个 failure
        failure_match = re.search(r"scm_error_budget_failure_count (\d+)", prom_output)
        if failure_match:
            assert int(failure_match.group(1)) >= 1


# ============ circuit_breaker key/scope 一致性测试 ============


class TestCircuitBreakerKeyConsistency:
    """测试 circuit_breaker key/scope 解析与 build_circuit_breaker_key 规范一致"""

    def test_global_scope_key_parsed_correctly(self, db_conn):
        """验证 global scope key 正确解析"""
        # 使用 build_circuit_breaker_key 创建 global scope 的 key
        cb_key = build_circuit_breaker_key("myproject", "global")
        cb_state = {"state": "closed", "failure_count": 0, "success_count": 5}
        save_circuit_breaker_state(db_conn, cb_key, cb_state)
        ...
        global_scope = by_scope["global"]
        assert global_scope["count"] >= 1
        assert global_scope["closed_count"] >= 1

    def test_instance_scope_key_parsed_correctly(self, db_conn):
        """验证 instance scope key 正确解析"""
        # 使用 build_circuit_breaker_key 创建 instance scope 的 key
        cb_key = build_circuit_breaker_key(
            project_key="myproject",
            instance_key="gitlab.example.com",
        )
        cb_state = {"state": "open", "failure_count": 10, "success_count": 2}
        save_circuit_breaker_state(db_conn, cb_key, cb_state)
        ...
        # 应该有 instance scope
        assert "instance" in by_scope
        instance_scope = by_scope["instance"]
        assert instance_scope["count"] >= 1
        assert instance_scope["open_count"] >= 1
        assert instance_scope["total_failures"] >= 10

    def test_tenant_scope_key_parsed_correctly(self, db_conn):
        """验证 tenant scope key 正确解析"""
        # 使用 build_circuit_breaker_key 创建 tenant scope 的 key
        cb_key = build_circuit_breaker_key(
            project_key="myproject",
            tenant_id="tenant-abc",
        )
        cb_state = {"state": "half_open", "failure_count": 5, "success_count": 1}
        save_circuit_breaker_state(db_conn, cb_key, cb_state)
        ...
        # 应该有 tenant scope
        assert "tenant" in by_scope
        tenant_scope = by_scope["tenant"]
        assert tenant_scope["count"] >= 1
        assert tenant_scope["half_open_count"] >= 1

    def test_pool_scope_key_parsed_correctly(self, db_conn):
        """验证 pool scope key 正确解析"""
        # 使用 build_circuit_breaker_key 创建 pool scope 的 key
        cb_key = build_circuit_breaker_key(
            project_key="myproject",
            worker_pool="gitlab-prod",
        )
        cb_state = {"state": "closed", "failure_count": 2, "success_count": 20}
        save_circuit_breaker_state(db_conn, cb_key, cb_state)
        ...
        # 应该有 pool scope
        assert "pool" in by_scope
        pool_scope = by_scope["pool"]
        assert pool_scope["count"] >= 1

    def test_circuit_breaker_total_counts(self, db_conn):
        """验证 circuit_breakers 的 total 计数正确"""
        # 插入多个不同状态的熔断器
        keys_and_states = [
            (build_circuit_breaker_key("proj1", "global"), {"state": "open"}),
            (build_circuit_breaker_key("proj2", instance_key="gitlab1.com"), {"state": "open"}),
            (
                build_circuit_breaker_key("proj3", instance_key="gitlab2.com"),
                {"state": "half_open"},
            ),
            (build_circuit_breaker_key("proj4", tenant_id="tenant-x"), {"state": "closed"}),
        ]

        for cb_key, cb_state in keys_and_states:
            save_circuit_breaker_state(db_conn, cb_key, cb_state)
        ...
        # 验证总计数
        assert circuit_breakers["total_count"] >= 4
        assert circuit_breakers["total_open"] >= 2  # 2 个 open 状态
        assert circuit_breakers["total_half_open"] >= 1  # 1 个 half_open 状态

    def test_parsed_key_included_in_entries(self, db_conn):
        """验证 entries 中包含解析后的 key 信息"""
        cb_key = build_circuit_breaker_key(
            project_key="parsed_test",
            instance_key="parsed.gitlab.com",
        )
        save_circuit_breaker_state(db_conn, cb_key, {"state": "closed"})
        ...
        # 找到 instance scope 中的 entries
        if "instance" in by_scope:
            entries = by_scope["instance"]["entries"]
            found = [e for e in entries if e["key"] == cb_key]

            if found:
                entry = found[0]
                # 验证包含 parsed_key 信息
                assert "parsed_key" in entry
                parsed = entry["parsed_key"]
                assert parsed["project_key"] == "parsed_test"
                assert parsed["scope_type"] == "instance"
                assert parsed["scope_value"] == "parsed.gitlab.com"


# ============ rate_limit_buckets 暂停语义测试 ============


class TestRateLimitBucketsPauseSemantics:
    """测试 rate_limit_buckets 暂停语义与 policy 的 InstanceBucketStatus 一致"""

    def test_paused_bucket_is_paused_true(self, db_conn):
        """验证 paused_until 在未来时 is_paused=True"""
        instance_key = "pause-semantics-test.com"
        with db_conn.cursor() as cur:
            cur.execute(
                """
                INSERT INTO scm.sync_rate_limits
                    (instance_key, tokens, rate, burst, paused_until, updated_at)
        ...
        bucket = found[0]
        # 验证暂停语义
        assert bucket["is_paused"] is True
        assert bucket["pause_remaining_seconds"] > 0
        assert bucket["pause_until"] is not None

    def test_expired_pause_is_paused_false(self, db_conn):
        """验证 paused_until 在过去时 is_paused=False"""
        instance_key = "expired-pause-test.com"
        with db_conn.cursor() as cur:
            cur.execute(
                """
                INSERT INTO scm.sync_rate_limits
        ...
        bucket = found[0]
        # 验证已过期的暂停
        assert bucket["is_paused"] is False
        assert bucket["pause_remaining_seconds"] == 0.0

    def test_null_pause_is_paused_false(self, db_conn):
        """验证 paused_until=NULL 时 is_paused=False"""
        instance_key = "null-pause-test.com"
        with db_conn.cursor() as cur:
            cur.execute(
                """
                INSERT INTO scm.sync_rate_limits
        ...
        # 验证 NULL 暂停
        assert bucket["is_paused"] is False
        assert bucket["pause_until"] is None

    def test_wait_seconds_calculated_correctly(self, db_conn):
        """验证 wait_seconds（令牌不足时的等待时间）计算正确"""
        instance_key = "wait-seconds-test.com"
        # tokens=0.5, rate=1.0 => 需要 0.5 秒才能获得 1 个令牌
        with db_conn.cursor() as cur:
            cur.execute(
                """
                INSERT INTO scm.sync_rate_limits
        ...
        bucket = found[0]
        # wait_seconds = (1.0 - 0.5) / 1.0 = 0.5
        assert 0.4 <= bucket["wait_seconds"] <= 0.6

    def test_bucket_status_matches_policy_semantics(self, db_conn):
        """验证 bucket 状态与 policy InstanceBucketStatus 语义一致"""
        from src.engram.logbook.scm_sync_policy import InstanceBucketStatus

        instance_key = "policy-match-test.com"
        with db_conn.cursor() as cur:
            cur.execute(
                """
                INSERT INTO scm.sync_rate_limits
                    (instance_key, tokens, rate, burst, paused_until, updated_at)
                VALUES (%s, %s, %s, %s, now() + interval '30 minutes', now())
                ON CONFLICT (instance_key) DO UPDATE
                SET tokens = EXCLUDED.tokens,
                    paused_until = EXCLUDED.paused_until,
                    updated_at = EXCLUDED.updated_at
            """,
                (instance_key, 5.0, 10.0, 20),
            )
        ...
        # 模拟创建 InstanceBucketStatus
        # 从 status 输出的数据应该能正确映射到 policy 的 InstanceBucketStatus
        policy_status = InstanceBucketStatus(
            instance_key=bucket["instance_key"],
            is_paused=bucket["is_paused"],
            pause_remaining_seconds=bucket["pause_remaining_seconds"],
            current_tokens=bucket["tokens_remaining"],
            rate=bucket["rate"],
            burst=float(bucket["burst"]),
        )

        # 验证映射后的语义一致
        assert policy_status.is_paused == bucket["is_paused"]
        assert policy_status.current_tokens == bucket["tokens_remaining"]
        assert policy_status.rate == bucket["rate"]

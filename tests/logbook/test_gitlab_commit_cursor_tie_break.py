...
class TestShouldAdvanceGitlabCommitCursor:
    ...
    def test_first_sync_always_advances(self):
        """首次同步总是推进"""
        assert (
            should_advance_gitlab_commit_cursor("2024-01-15T12:00:00Z", "abc123", None, None)
            is True
        )
...
class TestCommitCursorTieBreakIntegration:
    ...
    def test_same_second_multiple_commits_cursor_advances(self):
        ...
        for commit in sorted_commits:
            ...
            if should_advance_gitlab_commit_cursor(commit_ts_str, commit.sha, last_ts, last_sha):
...
class TestTimezoneFormatEquivalence:
    ...
    def test_mixed_format_cursor_storage(self):
        ...
        # 等价时间 + 相同 sha：不推进
        assert (
            should_advance_gitlab_commit_cursor(new_ts_same, cursor_sha, cursor_ts, cursor_sha)
            is False
        )

        # 等价时间 + 不同 sha：按 sha 比较
        assert (
            should_advance_gitlab_commit_cursor(new_ts_same, "def456", cursor_ts, cursor_sha)
            is True
        )

        # 晚 1 秒：总是推进
        assert (
            should_advance_gitlab_commit_cursor(new_ts_later, cursor_sha, cursor_ts, cursor_sha)
            is True
        )

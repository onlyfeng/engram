...
pytestmark = pytest.mark.skipif(HTTP_ONLY_MODE, reason=HTTP_ONLY_SKIP_REASON)
    ...
    def test_success_path_status_transition(self, migrated_db, logbook_adapter_config):
        ...
        try:
            ...
            mock_client.store.return_value = MockStoreResult(
                success=True, memory_id="mem_integration_success_001"
            )
            ...
            with conn.cursor() as cur:
                ...
                assert row[3] == "memory_id=mem_integration_success_001", (
                    f"last_error 应包含 memory_id，实际: {row[3]}"
                )
            ...
            if outbox_id:
                with conn.cursor() as cur:
                    ...
                    cur.execute(
                        f"DELETE FROM {governance_schema}.write_audit WHERE payload_sha = 'sha_success_001'"
                    )
            conn.close()

    def test_success_path_openmemory_called_with_correct_params(
        self, migrated_db, logbook_adapter_config
    ):
        ...
        try:
            ...
            if outbox_id:
                with conn.cursor() as cur:
                    ...
                    cur.execute(
                        f"DELETE FROM {governance_schema}.write_audit WHERE payload_sha = 'sha_team_params_002'"
                    )
            conn.close()
...
class TestOutboxWorkerIntegrationRetry:
    ...
    def test_retry_path_status_and_retry_count(self, migrated_db, logbook_adapter_config):
        ...
        try:
            ...
            mock_client = MagicMock()
            mock_client.store.return_value = MockStoreResult(
                success=False, error="connection_timeout"
            )
            ...
            with patch(
                ...
                config = WorkerConfig(
                    batch_size=10, max_retries=3, jitter_factor=0.0, base_backoff_seconds=60
                )
            ...
            with conn.cursor() as cur:
                ...
                assert audit_row[1] == "outbox_flush_retry", (
                    f"reason 应为 outbox_flush_retry，实际: {audit_row[1]}"
                )
            ...
            if outbox_id:
                with conn.cursor() as cur:
                    ...
                    cur.execute(
                        f"DELETE FROM {governance_schema}.write_audit WHERE payload_sha = 'sha_retry_003'"
                    )
            conn.close()
    ...
    def test_retry_path_becomes_dead_after_max_retries(self, migrated_db, logbook_adapter_config):
        ...
        try:
            ...
            mock_client = MagicMock()
            mock_client.store.return_value = MockStoreResult(
                success=False, error="permanent_failure"
            )
            ...
            with conn.cursor() as cur:
                ...
                assert audit_row[1] == "outbox_flush_dead", (
                    f"reason 应为 outbox_flush_dead，实际: {audit_row[1]}"
                )
            ...
            if outbox_id:
                with conn.cursor() as cur:
                    ...
                    cur.execute(
                        f"DELETE FROM {governance_schema}.write_audit WHERE payload_sha = 'sha_dead_004'"
                    )
            conn.close()
...
class TestOutboxWorkerIntegrationDedup:
    ...
    def test_dedup_path_skips_openmemory_call(self, migrated_db, logbook_adapter_config):
        ...
        try:
            ...
            mock_client = MagicMock()
            mock_client.store.return_value = MockStoreResult(
                success=True, memory_id="should_not_be_called"
            )
            ...
            with conn.cursor() as cur:
                ...
                assert row[3] == "memory_id=mem_original_005", (
                    f"last_error 应包含原始 memory_id，实际: {row[3]}"
                )
            ...
            with conn.cursor() as cur:
                ...
                cur.execute(
                    f"DELETE FROM {governance_schema}.write_audit WHERE payload_sha = 'sha_dedup_005'"
                )
            conn.close()

    def test_dedup_path_audit_contains_original_outbox_id(
        self, migrated_db, logbook_adapter_config
    ):
        ...
        try:
            ...
            with conn.cursor() as cur:
                ...
                cur.execute(
                    f"DELETE FROM {governance_schema}.write_audit WHERE payload_sha = 'sha_dedup_alice_006'"
                )
            conn.close()
        ...
        try:
            ...
            if outbox_id:
                with conn.cursor() as cur:
                    ...
                    cur.execute(
                        f"DELETE FROM {governance_schema}.write_audit WHERE payload_sha = 'sha_lock_007'"
                    )
            conn.close()
...
class TestOutboxWorkerIntegrationAuditValidation:
    ...
    def test_audit_action_reason_convention(self, migrated_db, logbook_adapter_config):
        ...
        try:
            ...
            # 按顺序处理每条记录
            for i, (payload_sha, will_succeed, _, expected_action, expected_reason) in enumerate(
                test_cases
            ):
                mock_client = MagicMock()
                if will_succeed:
                    mock_client.store.return_value = MockStoreResult(
                        success=True, memory_id=f"mem_{payload_sha}"
                    )
                else:
                    mock_client.store.return_value = MockStoreResult(
            ...
            for payload_sha, _, _, expected_action, expected_reason in test_cases:
                with conn.cursor() as cur:
                    ...
                    assert row[0] == expected_action, (
                        f"{payload_sha}: action 应为 {expected_action}，实际: {row[0]}"
                    )
                    assert row[1] == expected_reason, (
                        f"{payload_sha}: reason 应为 {expected_reason}，实际: {row[1]}"
                    )
                    assert row[1].startswith("outbox_flush_"), "reason 应以 'outbox_flush_' 开头"
            ...
            for payload_sha, _, _, _, _ in test_cases:
                with conn.cursor() as cur:
                    cur.execute(
                        f"DELETE FROM {governance_schema}.write_audit WHERE payload_sha = %s",
                        (payload_sha,),
                    )
            conn.close()
    ...
    def test_conflict_when_lease_stolen_by_second_worker(self, migrated_db, logbook_adapter_config):
        ...
        try:
            ...
            assert result.reason == "outbox_flush_conflict", (
                f"reason 应为 outbox_flush_conflict，实际: {result.reason}"
            )
            ...
            if outbox_id:
                with conn.cursor() as cur:
                    ...
                    cur.execute(
                        f"DELETE FROM {governance_schema}.write_audit WHERE payload_sha = 'sha_conflict_100'"
                    )
            conn.close()
        ...
        try:
            ...
            if outbox_id:
                with conn.cursor() as cur:
                    ...
                    cur.execute(
                        f"DELETE FROM {governance_schema}.write_audit WHERE payload_sha = 'sha_retry_conflict_101'"
                    )
            conn.close()
        ...
        try:
            ...
            if outbox_id:
                with conn.cursor() as cur:
                    ...
                    cur.execute(
                        f"DELETE FROM {governance_schema}.write_audit WHERE payload_sha = 'sha_no_dup_102'"
                    )
            conn.close()
        ...
        try:
            ...
            def run_worker1():
                try:
                    ...
                    with patch(
                        ...
                        worker1_results["results"] = process_batch(
                            config, worker_id="worker-1-lease"
                        )
            ...
            if not worker2_results["claimed"]:
                ...
                assert len(success_audits) == 1, (
                    f"应有 1 条 success 审计，实际: {len(success_audits)}"
                )
                assert len(conflict_audits) == 0, (
                    f"应无 conflict 审计，实际: {len(conflict_audits)}"
                )
            ...
            # 场景 B：Worker2 claim 到任务（租约过期被抢占）
                ...
                if len(w1_conflicts) > 0:
                    ...
                    if w2_results:
                        ...
                        assert len(w2_successes) == 1, (
                            f"Worker2 应成功完成，实际成功数: {len(w2_successes)}"
                        )
                    ...
                    assert len(success_audits) == 1, (
                        f"应只有 1 条 success 审计，实际: {len(success_audits)}"
                    )
            ...
            if outbox_id:
                with conn.cursor() as cur:
                    ...
                    cur.execute(
                        f"DELETE FROM {governance_schema}.write_audit WHERE payload_sha = 'sha_lease_conflict_001'"
                    )
            conn.close()
    ...
    def test_sequential_lease_conflict_deterministic(self, migrated_db, logbook_adapter_config):
        ...
        try:
            ...
            claimed_items = logbook_adapter.claim_outbox(
                worker_id="worker-1-seq", limit=10, lease_seconds=60
            )
            ...
            claimed_by_worker2 = logbook_adapter.claim_outbox(
                worker_id="worker-2-seq", limit=10, lease_seconds=60
            )

            assert len(claimed_by_worker2) == 1, (
                f"Worker2 应 claim 到 1 条（租约过期），实际: {len(claimed_by_worker2)}"
            )
            ...
            assert current_locked_by == "worker-2-seq", (
                f"locked_by 应为 worker-2-seq，实际: {current_locked_by}"
            )
            ...
            ack_result_2 = logbook_adapter.ack_sent(
                outbox_id=outbox_id, worker_id="worker-2-seq", memory_id="mem_worker2_success"
            )
            ...
            if outbox_id:
                with conn.cursor() as cur:
                    ...
                    cur.execute(
                        f"DELETE FROM {governance_schema}.write_audit WHERE payload_sha = 'sha_seq_lease_002'"
                    )
            conn.close()
...
class TestOutboxWorkerLeaseRenewal:
    ...
    def test_renew_lease_prevents_reclaim_during_slow_store(
        self, migrated_db, logbook_adapter_config
    ):
        """
        验证续期防止被 reclaim：
        ...
        try:
            ...
            if outbox_id:
                with conn.cursor() as cur:
                    ...
                    cur.execute(
                        f"DELETE FROM {governance_schema}.write_audit WHERE payload_sha = 'sha_lease_renew_001'"
                    )
            conn.close()
    ...
    def test_renew_lease_called_before_and_after_store(self, migrated_db, logbook_adapter_config):
        ...
        try:
            ...
            mock_client = MagicMock()
            mock_client.store.return_value = MockStoreResult(
                success=True, memory_id="mem_renew_002"
            )
            ...
            with patch(
                ...
                with patch(
                    "engram.gateway.outbox_worker.logbook_adapter.renew_lease",
            ...
            if outbox_id:
                with conn.cursor() as cur:
                    ...
                    cur.execute(
                        f"DELETE FROM {governance_schema}.write_audit WHERE payload_sha = 'sha_renew_track_002'"
                    )
            conn.close()
    ...
    def test_second_worker_cannot_reclaim_after_renew(self, migrated_db, logbook_adapter_config):
        ...
        try:
            ...
            worker2_items = logbook_adapter.claim_outbox(
                worker_id="worker-2", limit=10, lease_seconds=60
            )
            ...
            # Worker 1 仍能成功 ack
            ack_result = logbook_adapter.ack_sent(
                outbox_id, worker_id="worker-1", memory_id="mem_reclaim_003"
            )
            ...
            if outbox_id:
                with conn.cursor() as cur:
                    ...
                    cur.execute(
                        f"DELETE FROM {governance_schema}.write_audit WHERE payload_sha = 'sha_reclaim_003'"
                    )
            conn.close()

    def test_expired_lease_can_be_reclaimed_without_renew(
        self, migrated_db, logbook_adapter_config
    ):
        """
...
class TestOutboxWorkerSlowStoreTimeout:
    ...
    def test_slow_store_respects_timeout_no_internal_retry(
        self, migrated_db, logbook_adapter_config
    ):
        """
        验证长延迟 store 场景下：
        ...
        try:
            ...
            if outbox_id:
                with conn.cursor() as cur:
                    ...
                    cur.execute(
                        f"DELETE FROM {governance_schema}.write_audit WHERE payload_sha = 'sha_slow_store_001'"
                    )
            conn.close()
        ...
        try:
            ...
            def timeout_store(*args, **kwargs):
                ...
                raise OpenMemoryConnectionError(
                    message="Connection timeout after 1.0s", status_code=None, response=None
                )
            ...
            if outbox_id:
                with conn.cursor() as cur:
                    ...
                    cur.execute(
                        f"DELETE FROM {governance_schema}.write_audit WHERE payload_sha = 'sha_timeout_001'"
                    )
            conn.close()

    def test_processing_time_with_multiple_retries_controlled(
        self, migrated_db, logbook_adapter_config
    ):
        """
        ...
        try:
            ...
            if outbox_id:
                with conn.cursor() as cur:
                    ...
                    cur.execute(
                        f"DELETE FROM {governance_schema}.write_audit WHERE payload_sha = 'sha_controlled_001'"
                    )
            conn.close()
...
class TestOutboxWorkerDatabaseTimeout:
    ...
    def test_db_timeout_triggers_recoverable_path(
        self, migrated_db, logbook_adapter_config, monkeypatch
    ):
        """
        数据库超时触发可恢复路径：
        ...
        try:
            ...
            assert result.reason == "outbox_flush_db_timeout", (
                f"reason 应为 outbox_flush_db_timeout，实际: {result.reason}"
            )
            ...
            if outbox_id:
                with conn.cursor() as cur:
                    ...
                    cur.execute(
                        f"DELETE FROM {governance_schema}.write_audit WHERE payload_sha = 'sha_db_timeout_001'"
                    )
            conn.close()

    def test_outbox_recoverable_after_db_timeout(
        self, migrated_db, logbook_adapter_config, monkeypatch
    ):
        """
        ...
        try:
            ...
            mock_client = MagicMock()
            mock_client.store.return_value = MockStoreResult(
                success=True, memory_id="mem_recover_001"
            )
            ...
            if outbox_id:
                with conn.cursor() as cur:
                    ...
                    cur.execute(
                        f"DELETE FROM {governance_schema}.write_audit WHERE payload_sha = 'sha_recover_001'"
                    )
            conn.close()
    ...
    def test_db_error_classified_correctly(self, migrated_db, logbook_adapter_config, monkeypatch):
        ...
        try:
            ...
            assert result.reason == "outbox_flush_db_error", (
                f"reason 应为 outbox_flush_db_error，实际: {result.reason}"
            )
            ...
            if outbox_id:
                with conn.cursor() as cur:
                    ...
                    cur.execute(
                        f"DELETE FROM {governance_schema}.write_audit WHERE payload_sha = 'sha_db_error_001'"
                    )
            conn.close()
...
class TestOutboxDegradationRecoveryE2E:
    ...
    def test_degradation_to_outbox_recovery_flush_audit_consistency(
        self, migrated_db, logbook_adapter_config
    ):
        """
        ...
        try:
            ...
            with conn.cursor() as cur:
                ...
                """,
                    (
                        ...
                        '{"outbox_id": '
                        + str(outbox_id)
                        + ', "source": "gateway", "extra": {"correlation_id": "e2e-test"}}',
                    ),
                )
            ...
            mock_client = MagicMock()
            mock_client.store.return_value = MockStoreResult(
                success=True, memory_id="mem_e2e_recovery_001"
            )
            ...
            assert our_result.reason == "outbox_flush_success", (
                f"reason 应为 outbox_flush_success，实际: {our_result.reason}"
            )
            ...
            with conn.cursor() as cur:
                ...
                assert audit_row is not None, "应存在 reason=outbox_flush_success 的审计记录"
                assert audit_row[0] == "allow", f"action 应为 allow，实际: {audit_row[0]}"
                assert audit_row[1] == "outbox_flush_success", (
                    f"reason 应为 outbox_flush_success，实际: {audit_row[1]}"
                )
    ...
    def test_degradation_max_retries_dead_audit_consistency(
        self, migrated_db, logbook_adapter_config
    ):
        """
        测试超过最大重试次数后的审计一致性
        ...
        try:
            ...
            mock_client = MagicMock()
            mock_client.store.return_value = MockStoreResult(
                success=False, error="openmemory_still_unavailable"
            )
    ...
    def test_reconcile_audit_reason_matches_outbox_final_status(
        self, migrated_db, logbook_adapter_config
    ):
        """
        ...
        try:
            ...
            with conn.cursor() as cur:
                ...
                assert sent_audit[0] == "allow", (
                    f"sent 审计 action 应为 allow，实际: {sent_audit[0]}"
                )
            ...
            with conn.cursor() as cur:
                ...
                assert dead_audit[0] == "reject", (
                    f"dead 审计 action 应为 reject，实际: {dead_audit[0]}"
                )
...
class TestOutboxWorkerFullAcceptance:
    ...
    def test_full_acceptance_status_transitions(self, migrated_db, logbook_adapter_config):
        ...
        try:
            ...
            with patch(
                ...
                config = WorkerConfig(batch_size=10, max_retries=5, jitter_factor=0.0)
                process_batch(config, worker_id="full-acceptance-worker")
            ...
            with conn.cursor() as cur:
                ...
                cur.execute(
                    f"DELETE FROM {governance_schema}.write_audit WHERE payload_sha LIKE '{test_prefix}%%'"
                )
            conn.close()
    ...
    def test_full_acceptance_audit_reason_values(self, migrated_db, logbook_adapter_config):
        ...
        try:
            ...
            with conn.cursor() as cur:
                ...
                assert row[1] == "outbox_flush_success", (
                    f"[审计 reason] success reason 应为 outbox_flush_success，实际: {row[1]}"
                )
                ...
                assert row[0] == "redirect", (
                    f"[审计 reason] retry action 应为 redirect，实际: {row[0]}"
                )
                assert row[1] == "outbox_flush_retry", (
                    f"[审计 reason] retry reason 应为 outbox_flush_retry，实际: {row[1]}"
                )
                ...
                assert row[1] == "outbox_flush_dead", (
                    f"[审计 reason] dead reason 应为 outbox_flush_dead，实际: {row[1]}"
                )
    ...
    def test_full_acceptance_evidence_refs_outbox_id_queryable(
        self, migrated_db, logbook_adapter_config
    ):
        """
        ...
        try:
            ...
            mock_client = MagicMock()
            mock_client.store.return_value = MockStoreResult(
                success=True, memory_id=f"mem_{test_prefix}"
            )
            ...
            with conn.cursor() as cur:
                ...
                assert row[0] == "allow", (
                    f"[evidence_refs_json 查询] action 应为 allow，实际: {row[0]}"
                )
                assert row[1] == "outbox_flush_success", (
                    f"[evidence_refs_json 查询] reason 应为 outbox_flush_success，实际: {row[1]}"
                )
                assert row[2] == test_prefix, (
                    f"[evidence_refs_json 查询] payload_sha 应匹配，实际: {row[2]}"
                )
                ...
                assert "correlation_id" in extra, (
                    "[evidence_refs_json 查询] extra 应包含 correlation_id"
                )
            ...
            if outbox_id:
                with conn.cursor() as cur:
                    ...
                    cur.execute(
                        f"DELETE FROM {governance_schema}.write_audit WHERE payload_sha = '{test_prefix}'"
                    )
                    cur.execute(
                        f"""
                        DELETE FROM {governance_schema}.write_audit

...
class TestReconcileSentRecords:
    ...
    def test_detect_sent_missing_audit(self, db_conn_for_reconcile, reconcile_env):
        ...
        assert result.sent_missing_audit >= 1, (
            f"应检测到至少 1 条缺失审计，实际: {result.sent_missing_audit}"
        )
    ...
    def test_sent_audit_queryable_after_fix(self, db_conn_for_reconcile, reconcile_env):
        ...
        with conn.cursor() as cur:
            ...
            assert (
                evidence_refs_json.get("gateway_event", {}).get("outbox_id") == outbox_id
                or evidence_refs_json.get("outbox_id") == outbox_id
...
class TestReconcileDeadRecords:
    ...
    def test_detect_dead_missing_audit(self, db_conn_for_reconcile, reconcile_env):
        ...
        # 验证检测到缺失
        assert result.dead_missing_audit >= 1, (
            f"应检测到至少 1 条缺失审计，实际: {result.dead_missing_audit}"
        )
...
class TestReconcileStaleRecords:
    ...
    def test_detect_stale_locked_record(self, db_conn_for_reconcile, reconcile_env):
        ...
        assert result.stale_missing_audit >= 1, (
            f"应检测到至少 1 条缺失审计，实际: {result.stale_missing_audit}"
        )
        assert result.stale_audit_fixed >= 1, (
            f"应修复至少 1 条审计，实际: {result.stale_audit_fixed}"
        )
    ...
    def test_stale_audit_queryable_after_fix(self, db_conn_for_reconcile, reconcile_env):
        ...
        with conn.cursor() as cur:
            ...
            assert reason.startswith("outbox_stale"), f"reason '{reason}' 应以 'outbox_stale' 开头"
            # 验证 evidence_refs_json 包含正确的 outbox_id
            assert (
                evidence_refs_json.get("gateway_event", {}).get("outbox_id") == outbox_id
                or evidence_refs_json.get("outbox_id") == outbox_id
    ...
    def test_stale_reschedule(self, db_conn_for_reconcile, reconcile_env):
        ...
        # 验证重新调度
        assert result.stale_rescheduled >= 1, (
            f"应重新调度至少 1 条记录，实际: {result.stale_rescheduled}"
        )
...
class TestReconcileReasonErrorCodeContract:
    ...
    def test_reconcile_uses_errorcode_constants_for_sent(
    ...
    def test_reconcile_uses_errorcode_constants_for_dead(
        self, db_conn_for_reconcile, reconcile_env
    ):
    ...
    def test_reconcile_uses_errorcode_constants_for_stale(
        self, db_conn_for_reconcile, reconcile_env
    ):
    ...
    def test_all_outbox_errorcodes_defined_in_logbook(self):
        ...
        for code_name in expected_outbox_errorcodes:
            assert hasattr(ErrorCode, code_name), f"ErrorCode 应定义 {code_name}，但未找到"
    ...
    def test_reconcile_audit_reason_matches_outbox_status(
        self, db_conn_for_reconcile, reconcile_env
    ):
        """
        ...
        for status, reasons in status_to_reason_mapping.items():
            for reason in reasons:
                assert isinstance(reason, str), f"状态 '{status}' 的 reason '{reason}' 应为字符串"
...
class TestReconcileSmokeTest:
    ...
    def test_reconcile_cli_once_exit_code(self, reconcile_env, is_full_profile):
        ...
        result = subprocess.run(
            [
                sys.executable,
                "-m",
                "engram.gateway.reconcile_outbox",
                "--once",
                "--scan-window",
                "1",  # 仅扫描最近 1 小时
                "--batch-size",
    ...
    def test_reconcile_cli_summary_format(self, reconcile_env, is_full_profile):
        ...
        assert "Total scanned:" in stdout, f"摘要应包含 'Total scanned:'\nstdout: {stdout}"

        assert "sent:" in stdout, f"摘要应包含 'sent:' 统计\nstdout: {stdout}"

        assert "dead:" in stdout, f"摘要应包含 'dead:' 统计\nstdout: {stdout}"

        assert "stale:" in stdout, f"摘要应包含 'stale:' 统计\nstdout: {stdout}"
...
class TestAuditOutboxInvariants:
    ...
    def test_redirect_to_outbox_enqueue_audit_invariant(self, db_conn_for_reconcile, reconcile_env):
        """
        ...
        with conn.cursor() as cur:
            ...
            assert row[1] == "team_write_disabled", (
                f"reason 应为 'team_write_disabled'，实际: {row[1]}"
            )
    ...
    def test_outbox_flush_success_audit_invariant(self, db_conn_for_reconcile, reconcile_env):
        """
        ...
        assert action == "allow", f"sent → 审计 action 应为 'allow'，实际: {action}"
        ...
        assert "source" in evidence_refs_json, "evidence_refs_json 顶层必须包含 source"
        ...
        assert gateway_event.get("outbox_id") == outbox_id, "gateway_event.outbox_id 应与顶层一致"

    def test_outbox_flush_dead_audit_invariant(self, db_conn_for_reconcile, reconcile_env):
        """
        ...
        assert action == "reject", f"dead → 审计 action 应为 'reject'，实际: {action}"
        ...
        assert "outbox_id" in evidence_refs_json, "evidence_refs_json 顶层必须包含 outbox_id"
        assert evidence_refs_json["outbox_id"] == outbox_id
        ...
        assert "extra" in evidence_refs_json, "evidence_refs_json 应包含 extra 字段"
        assert "last_error" in evidence_refs_json["extra"], "extra 应包含 last_error 字段"

    def test_outbox_stale_audit_invariant(self, db_conn_for_reconcile, reconcile_env):
        """
        ...
        assert action == "redirect", f"stale → 审计 action 应为 'redirect'，实际: {action}"
    ...
    def test_full_redirect_outbox_reconcile_loop(self, db_conn_for_reconcile, reconcile_env):
        """
        ...
        with conn.cursor() as cur:
            ...
            assert row[1] == "team_write_disabled", (
                "redirect 审计 reason 应为 'team_write_disabled'"
            )
        ...
        with conn.cursor() as cur:
            ...
            assert action == "allow", f"sent → action 应为 'allow'，实际: {action}"
            assert reason == ErrorCode.OUTBOX_FLUSH_SUCCESS, (
    ...
    def test_evidence_refs_json_sql_query_contract(self, db_conn_for_reconcile, reconcile_env):
        """
        ...
        """
        conn = db_conn_for_reconcile
        from engram.gateway.reconcile_outbox import (
            ReconcileConfig,
            check_audit_exists,
            run_reconcile,
        )
        ...
        assert exists, "check_audit_exists 应能通过 evidence_refs_json->>'outbox_id' 找到审计记录"
        ...
        with conn.cursor() as cur:
            ...
            assert count >= 1, "SQL 查询契约: evidence_refs_json->>'outbox_id' 应能匹配审计记录"

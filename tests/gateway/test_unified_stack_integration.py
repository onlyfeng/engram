...
_WORKSPACE_ROOT = Path(__file__).resolve().parents[4]  # gateway/tests -> gateway -> openmemory_gateway -> apps -> workspace
...
class TestMockDegradationFlow:
    ...
    def test_mock_degradation_and_recovery(self, integration_config, postgres_connection):
        ...
        from engram.gateway.outbox_worker import process_batch, WorkerConfig
...
class TestStartupVerificationErrors:
    ...
    def test_gateway_health_check_provides_clear_status(
        ...
        assert data["service"] == "memory-gateway", f"健康检查 service 应为 'memory-gateway': {data}"

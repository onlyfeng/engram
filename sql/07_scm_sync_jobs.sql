...
-- ============================================================
-- 唯一约束：防止同一仓库同一任务类型的重复 pending/running 任务
-- ============================================================

-- 创建一个局部唯一索引：同一 (repo_id, job_type) 只能有一个 pending 或 running 状态的任务
CREATE UNIQUE INDEX IF NOT EXISTS idx_sync_jobs_unique_active
    ON scm.sync_jobs(repo_id, job_type)
    WHERE status IN ('pending', 'running');
